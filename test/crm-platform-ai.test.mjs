...
test("route CRM richiedono modulo e schermata di catalogo", () => {
  ...
  assert.match(crmModule, /moduleCode="crm_b2b" screenCode="crm\.b2b\.clienti"/);
  assert.match(crmModule, /moduleCode="crm_online" screenCode="crm\.online\.campaigns"/);
...
test("KPI senza fonte restano esplicitamente non disponibili", () => {
  assert.match(crmModule, /Dato non disponibile/);
  assert.match(crmModule, /Richiede connettore ADV autorizzato/);
