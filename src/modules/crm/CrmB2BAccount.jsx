...
export default function CrmB2BAccount({ account, metrics, related, commercialSnapshot: snapshot, journey, period, warning, canWrite, activity, setActivity, activityTemplates, activityBusy, addActivity, load, onStatus }) {
  ...
  const [extra, setExtra] = useState({ loading: true, invoices: [], orders: [], beauty: [], projects: [], errors: {} });
  const key = account.codice_cliente_mexal ? `mexal:${account.codice_cliente_mexal}` : account.entityKey;
  const purchased = useCustomerProductData(key, 'purchased', 'b2b'); const ordered = useCustomerProductData(key, 'ordered', 'b2b');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setExtra({ loading: true, invoices: [], orders: [], beauty: [], projects: [], errors: {} });
      const requests = {
        ...
        projects: loadAllQueryRows((from, to) => supabase.from('v4_progetti').select('id,titolo,stato,deadline,crm_customer_key').eq('crm_customer_key', key).order('id').range(from, to)).then(take),
        beauty: (async () => {
          if (!code) return [];
          const settings = await supabase.from('crm_workflow_settings').select('beauty_post_evento_giorni').eq('crm_tipo', 'b2b').maybeSingle();
  ...
  }, [account.codice_cliente_mexal, key, revision]);
  ...
  const activityRows = related.activities.map(r => ({ ...r, type: r.checklist_template?.titolo || r.tipo?.replaceAll('_', ' ') }));
  ...
  const operational = [
    card('beauty', 'Beauty Days', extra.loading ? '…' : extra.errors.beauty ? '—' : extra.beauty.length, 'Appuntamenti e storico del cliente', 'Giornate Beauty Days collegate all’anagrafica del cliente. Il popup comprende stato, consulente, pezzi, fatturato e impatto successivo.'),
    card('activities', 'Attività', activityRows.length, `${activityRows.filter(r => r.stato !== 'completata').length} aperte · ${activityRows.filter(r => r.stato === 'completata').length} completate`, 'Tutte le attività CRM del cliente. Le nuove attività provengono dalle checklist abilitate per B2B.'),
    card('timeline', 'Timeline cliente', journey.length, 'Eventi dello storico CRM', 'Eventi unificati del cliente, con data, tipologia e dettaglio, nel perimetro autorizzato.'),
  ...
  function detail() {
    ...
    } else if (id === 'activities' || id === 'overdue') {
      rows = activityRows.filter(r => id !== 'overdue' || r.stato !== 'completata' && r.data_attivita && new Date(r.data_attivita) < new Date());
      columns = [col('titolo', 'Attività'), col('type', 'Voce checklist'), date('data_attivita'), col('stato', 'Stato'), { key: 'actions', label: 'Azioni', value: () => '', render: r => <CrmDeleteActivityButton activity={r} canDelete={canWrite} onDeleted={load} onError={setActionError} compact/> }];
    } else if (id === 'timeline') { rows = journey.map((r, i) => ({ ...r, id: `${r.event_type}-${r.entity_id}-${i}` })); columns = [date('event_at'), col('title', 'Evento'), col('event_type', 'Tipo'), col('detail', 'Dettaglio')];
    ...
    } else if (id === 'opportunities' || id === 'weighted') { rows = related.opportunities.filter(r => !r.crm_opportunity_stages?.finale); columns = [col('titolo', 'Opportunità'), { key: 'stage', label: 'Fase', value: r => r.crm_opportunity_stages?.nome }, money('valore', 'Valore'), col('probabilita', 'Probabilità %'), { key: 'open', label: 'Dettaglio', value: () => 'Apri opportunità', render: r => <Link to={period.withPeriod(`/crm/b2b/pipeline/${r.id}`)}>Apri opportunità</Link> }];
    ...
    } else if (id === 'ai') { return <><p>La sintesi viene generata su richiesta nel perimetro autorizzato.</p><Link className="secondary-action" to="/crm/ai" state={{ accountId: account.crm_account_id, customerCode: account.codice_cliente_mexal, crmType: 'b2b' }}>Apri AI Brief</Link></>;
    } else if (id === 'account') {
      rows = Object.entries({ 'Ragione sociale': account.nome, 'Codice cliente': account.codice_cliente_mexal, 'Stato CRM': account.crm_active ? 'Attivo' : 'Non attivo', 'Stato relazione': account.stato, 'Agente': account.agente_nome, 'Area CRM': 'B2B', 'Classificazione': [account.origine_classificazione, account.modalita_classificazione].filter(Boolean).join(' · '), 'Partita IVA': account.partita_iva, 'Paese / nazionalità': account.paese, 'Email': account.email, 'Telefono': account.telefono, 'Indirizzo': [account.indirizzo, account.cap, account.citta, account.provincia].filter(Boolean).join(' · '), 'Valore CRM': formatMoney(account.valore_cliente) }).map(([name, value]) => ({ id: name, name, value: value || '—' })); columns = [col('name', 'Campo'), col('value', 'Valore')];
    }
    return <>{(id === 'activities' || id === 'overdue') && canWrite && <form className="crm-inline-form" onSubmit={addActivity}>
      <select required aria-label="Voce checklist B2B" value={activity.catalog_template_id} disabled={activityBusy || !activityTemplates.length} onChange={e => setActivity({ ...activity, catalog_template_id: e.target.value })}><option value="">{activityTemplates.length ? 'Seleziona voce checklist B2B' : 'Nessuna checklist abilitata per B2B'}</option>{activityTemplates.map(t => <option key={t.id} value={t.id}>{t.nome}</option>)}</select>
      <input required aria-label="Titolo attività" placeholder="Titolo attività" value={activity.titolo} onChange={e => setActivity({ ...activity, titolo: e.target.value })}/><input type="datetime-local" aria-label="Data attività" value={activity.data_attivita} onChange={e => setActivity({ ...activity, data_attivita: e.target.value })}/><button className="primary-action" disabled={activityBusy || !activity.catalog_template_id}>{activityBusy ? 'Salvataggio…' : 'Aggiungi attività'}</button>
    </form>}{loading ? <p role="status">Caricamento dettagli…</p> : error ? <p role="alert">{error}</p> : <CrmDetailTable rows={rows} columns={columns}/>}</>;
  }
  return <div className="crm-page b2b-account-page">
    <CrmPageHeader eyebrow="CRM B2B" title={account.nome} description={[account.citta, account.agente_nome].filter(Boolean).join(' · ')} actions={<><CrmPeriodFilter period={period} compact/><Link className="secondary-action" to={period.withPeriod('/crm/b2b/clienti')}>Torna ai clienti</Link></>}><CrmCustomerStatusBadge active={account.crm_active}/></CrmPageHeader>
    {warning && <p className="crm-message warning">{warning}</p>}
    <div className="b2b-account-actions"><button className="secondary-action" onClick={() => setSelected({ id: 'account', label: 'Anagrafica cliente' })}>Anagrafica</button>{canWrite && <><button className="primary-action" onClick={() => setSelected({ id: 'activities', label: 'Attività' })}>+ Attività</button>{hasPermission('projects.write') && <button className="secondary-action" onClick={() => setCreateProject(true)}>+ Progetto</button>}<button className="secondary-action" onClick={onStatus}>{account.crm_active ? 'Disattiva cliente' : 'Riattiva cliente'}</button></>}</div>
    ...
    <section className="b2b-kpi-section"><h2>2. Relazione commerciale e riordini</h2><div className="crm-kpi-grid">
      ...
      {card('reorder', 'Prossimo riordino atteso', formatDate(snapshot.b2b?.expected_reorder_date), snapshot.b2b?.classification?.replaceAll('_', ' ') || 'Prospect', 'Data attesa del prossimo riordino stimata dalla frequenza storica individuale.')}
    </div></section>
    ...
    <WorkspaceProjectCreateDialog open={createProject} crmType="b2b" initialCustomerKey={key} onClose={() => setCreateProject(false)} onSaved={() => { setCreateProject(false); setRevision(v => v + 1); load(); }}/>
  </div>;
}
