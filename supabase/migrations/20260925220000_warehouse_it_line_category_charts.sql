...
), it_classifications as (
  select dimension.kind, dimension.label, f.unit_of_measure,
    sum(greatest(f.on_hand,0))::numeric as quantity,
    case when scope.customer_code is null then sum(greatest(f.on_hand,0)*f.unit_cost)::numeric else null end as stock_value
  from filtered f
  cross join customer_scope scope
  left join public.prodotti p on p.codice_mexal=f.article_code
  cross join lateral (values
    ('line',coalesce(nullif(btrim(p.linea_mexal),''),'Linea non assegnata')),
    ('category',coalesce(nullif(btrim(p.categoria_mexal),''),'Categoria non assegnata'))
  ) dimension(kind,label)
  where f.article_type='IT'
  group by dimension.kind,dimension.label,f.unit_of_measure,scope.customer_code
), it_breakdown as (
  select jsonb_build_object(
    'byLine',coalesce(jsonb_agg(to_jsonb(c)-'kind'-case when c.stock_value is null then 'stock_value' else 'no_field' end order by c.label,c.unit_of_measure) filter(where c.kind='line'),'[]'::jsonb),
    'byCategory',coalesce(jsonb_agg(to_jsonb(c)-'kind'-case when c.stock_value is null then 'stock_value' else 'no_field' end order by c.label,c.unit_of_measure) filter(where c.kind='category'),'[]'::jsonb)
  ) as value from it_classifications c
), page_rows as (
...
select jsonb_build_object(
  ...
  'itBreakdown', it_breakdown.value,
  'rows', page_rows.value
)
from parameters, customer_scope, available_dates, snapshot_status, totals, type_breakdown, warehouse_breakdown, unit_breakdown, page_rows, it_breakdown;
$function$;
