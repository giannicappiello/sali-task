...
export default function HrModule({ configuration = false }) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const result = sortHrPeople(await hrRpc('workspace_hr_snapshot', { p_month: `${month}-01`, p_config: configuration }));
  ...
  const tabs = configuration ? [['employees', UsersRound, 'Schede dipendente'], ['sites', MapPin, 'Sedi e timbrature'], ['recipients', UsersRound, 'Destinatari richieste'], ['export', CalendarDays, 'Esporta presenze'], ['economics', Timer, 'Riepilogo economico'], ['audit', Clock3, 'Storico modifiche']]
