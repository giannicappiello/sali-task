...
assert.deepEqual(payload, { sigla: "OC", serie: 2, numero: 0, cod_conto: "C1", data_documento: [[1, "20/07/2026"]], cod_modulo: "X", id_magazzino: 5, nota: [[1, "nota test"]], id_pagamento: 7, codice_articolo: [[1, "IT0058"], [2, "IT0204"]], quantita: [[1, 4], [2, 6]], prezzo: [[1, 15.68], [2, 12]], sconto: [[1, "50+35"], [2, "50+35"]], id_mag_riga: [[1, 5], [2, 5]], tp_um_articolo: [[1, "1"], [2, "1"]], cod_iva: [[1, "22,0"], [2, "22,0"]] });
const ocmPayload = buildMexalOrderDocument({ id: "workspace-1", codice_cliente: "C1", data_ordine: "2026-07-20" }, "OCM", classified.OCM);
...
assert.equal(DEFAULT_MEXAL_ORDER_DATE_FORMAT, "typed-array-dd/mm/yyyy", "the shared date format default uses the typed Mexal matrix");
assert.deepEqual(formatMexalOrderDate("2026-07-20"), [[1, "20/07/2026"]], "the default Mexal date format uses a typed matrix");
...
assert.equal(documentOptions({}, "OCM").dateFormat, "typed-array-dd/mm/yyyy", "unset environment uses the typed date-format default");
