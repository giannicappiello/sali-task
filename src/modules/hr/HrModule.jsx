...
import './hr.css';
import { downloadAttendanceWorkbook, downloadManagedOvertimeWorkbook, exportDate } from './hrAttendanceExport';
import { mapsSearchUrl, parseCoordinates } from './hrLocation';
import { sortHrPeople } from './hrPeople';
import { overtimeSummary } from './hrOvertimeSummary';
...
function AttendanceDayDialog({ detail, onClose }) {
  ...
  return <dialog ref={dialog} className="hr-dialog" onCancel={(event) => { event.preventDefault(); onClose(); }} aria-labelledby="hr-attendance-day-title">
    ...
    <section aria-labelledby="hr-attendance-present-title"><h3 id="hr-attendance-present-title">Presenti</h3>{detail.present.length ? detail.present.map(row => <div className="hr-day-detail-row" key={row.id}><strong>{detail.name(row.user_id)}</strong><span>Entrata: {formatTime(row.checkin_at)} · Uscita: {row.checkout_at ? formatTime(row.checkout_at) : 'Presenza ancora aperta'}</span>{row.exit_reason && <small>Motivo uscita: {row.exit_reason}</small>}</div>) : <Empty>Nessuna presenza registrata.</Empty>}</section>
  ...
  return <dialog ref={dialog} className="hr-dialog" onCancel={(event) => { event.preventDefault(); if (!saving) onClose(); }} aria-labelledby="hr-editor-title">
    <form onSubmit={submit}><header><h2 id="hr-editor-title">{model.title}</h2><button type="button" aria-label="Chiudi" onClick={onClose} disabled={saving}><X size={20}/></button></header>
      ...
      <div className="hr-form-grid">{model.fields.map((f) => f.type === 'multiselect' ? <fieldset key={f.key} className="hr-wide"><legend>{f.label}</legend><span className="hr-recipient-list">{f.options.map(([id, name]) => <label key={id}><input type="checkbox" checked={(values[f.key] || []).includes(id)} onChange={(e) => setValues(previous => ({ ...previous, [f.key]: e.target.checked ? [...(previous[f.key] || []), id] : previous[f.key].filter(value => value !== id) }))}/>{name}</label>)}</span></fieldset> : <label key={f.key} className={f.wide ? 'hr-wide' : ''}>{f.label}
        {f.type === 'checkbox' ? <input className="hr-agreement-checkbox" type="checkbox" checked={values[f.key] === true} onChange={(e) => setValues({ ...values, [f.key]: e.target.checked })}/> : f.type === 'select' ? <select required={f.required !== false} value={values[f.key] ?? ''} onChange={(e) => setValues({ ...values, [f.key]: e.target.value })}><option value="">Seleziona…</option>{f.options.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select>
      ...
      {model.siteLocation && <section className="hr-note"><h3>Rete aziendale</h3><p>Inserisci gli IP pubblici della sede, uno per riga. Questi IP restano disponibili per le integrazioni aziendali. Le timbrature usano la verifica GPS.</p><button type="button" disabled={saving} onClick={async () => { setSaving(true); setError(''); try { const result = await hrNetwork(); setValues(previous => ({ ...previous, public_ips: [...new Set([...(previous.public_ips || '').split(/[\s,;]+/).filter(Boolean), result.ip])].join('\n') })); } catch (failure) { setError(failure.message); } finally { setSaving(false); } }}>Usa IP attuale</button><p>Controlla l’elenco e premi Salva per applicare gli IP rilevati. Le timbrature non dipendono da questo elenco.</p></section>}
...
export default function HrModule({ configuration = false }) {
  ...
  const [page, setPage] = useState(configuration ? 'company-calendar' : ['leave','overtime'].includes(searchParams.get('richieste')) ? searchParams.get('richieste') : 'personal');
  ...
  const tabs = configuration ? [['company-calendar', CalendarDays, 'Calendario Aziendale'], ['economics', Timer, 'Riepilogo economico'], ['export', CalendarDays, 'Esporta presenze'], ['employees', UsersRound, 'Schede dipendente'], ['sites', MapPin, 'Sedi e timbrature']]
  ...
  function scheduleAbsence() {
    show({ title: 'Nuova assenza', submit: 'Salva assenza',
      ...
      async (values, key) => {
        const startsAt = romeInstant(values.starts_at);
        const endsAt = romeInstant(values.ends_at);
        if (!startsAt || !endsAt || Date.parse(endsAt) <= Date.parse(startsAt)) throw new Error('La data finale deve essere successiva alla data iniziale.');
        await hrRpc('workspace_hr_admin_request', { p_data: { ...values, starts_at: startsAt, ends_at: endsAt, request_key: key } });
        await load(true);
      });
  }
  function review(row) {
  ...
  const economicRows = configuration && page === 'economics' ? overtimeSummary(data, month).filter(row => visibleEmployees.some(employee => employee.user_id === row.user_id)) : [];
  ...
  const openAttendanceDay = (day, present, absent) => setAttendanceDayDetail({
    label: formatDisplayDate(new Date(`${day}T12:00:00Z`), { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' }),
    present, absent, name,
  });
  ...
  return <div className="hr-page">
    <WorkspacePageHeader icon={<UsersRound size={30}/>} eyebrow="Human Resources" title={configuration ? 'Rendicontazioni HR' : 'Presenze, turni e richieste'} description={configuration ? 'Schede dipendente e accordi contrattuali ed economici · Solo admin.' : 'Gestione del personale nel Workspace.'}/>
    ...
    {!configuration && data.member && <HrPunchButton className={`hr-full hr-punch-action ${open ? 'hr-checkout' : 'hr-primary'}`} disabled={busy || loading} onComplete={() => load(true)}/>}
    ...
    {loading ? <div className="hr-panel">Caricamento HR…</div> : !error && <>
      ...
      {configuration && page === 'sites' && <section className="hr-panel"><div className="hr-heading"><h2>Sedi e timbrature</h2><button className="hr-primary" onClick={() => site()}>Nuova sede</button></div>{data.sites.map((s) => <article className="hr-contract" key={s.id}><div className="hr-heading"><h3>{s.name}</h3><button onClick={() => site(s)}>Modifica</button></div><p>{s.address}</p><p><a href={mapsSearchUrl(s.latitude + ',' + s.longitude)} target="_blank" rel="noopener noreferrer">{s.latitude}, {s.longitude} · Apri Google Maps ↗</a></p><p><strong>Entrata e uscita: verifica GPS puntuale</strong></p><p>IP autorizzati: {s.public_ips?.length ? s.public_ips.join(', ') : 'Nessuno'}</p><p>Checkout automatico GPS oltre {s.checkout_radius} m · {s.auto_checkout ? 'Attivo' : 'Disattivo'}</p></article>)}{!data.sites.length && <Empty>Configura la sede e gli IP pubblici aziendali per abilitare le timbrature.</Empty>}<div className="hr-note">Ingresso con GPS preciso nel raggio della sede. Uscita manuale con posizione puntuale e motivazione obbligatoria fuori sede. La prima posizione attendibile oltre il raggio chiude automaticamente la presenza e segnala l’allontanamento. Il browser deve restare attivo: una posizione mancante non chiude la presenza.</div></section>}
      {configuration && page === 'export' && <section className="hr-panel"><h2>Foglio presenze per il consulente del lavoro</h2><p>Periodo selezionato: <strong>{exportDate(monthDays(month)[0])} – {exportDate(monthDays(month).at(-1))}</strong>. Esporta il solo foglio presenze mensili oppure il prospetto degli straordinari gestiti separatamente, con valorizzazione.</p><div className="hr-actions"><button className="hr-primary" disabled={busy || loading} onClick={async () => { setBusy(true); setError(''); try { await downloadAttendanceWorkbook(data,month); } catch (failure) { setError(failure.message); } finally { setBusy(false); } }}>{busy ? 'Preparazione Excel…' : 'Esporta presenze'}</button><button className="hr-primary" disabled={busy || loading} onClick={async () => { setBusy(true); setError(''); try { await downloadManagedOvertimeWorkbook(data,month); } catch (failure) { setError(failure.message); } finally { setBusy(false); } }}>Esporta straordinario gestito</button></div><p className="hr-note">Controlla le anomalie prima di inviare il file. Le uscite mancanti e i dati non determinabili restano evidenziati; le ore rilevate non equivalgono automaticamente alle ore retribuite.</p></section>}
      {configuration && page === 'economics' && <HrExcessOvertime month={month} employees={visibleEmployees} data={data} onConverted={() => load(true)}/>}
      {configuration && page === 'economics' && <section className="hr-panel"><h2>Straordinari approvati · {month}</h2><p className="hr-muted">Una riga per dipendente: totale delle ore approvate nel mese. Valore indicativo = ore × tariffa oraria × (1 + maggiorazione / 100). Si applica l’accordo valido nel giorno dello straordinario.</p>{economicRows.length ? <div className="hr-table-wrap"><table><thead><tr><th>Dipendente</th><th>Ore totali autorizzate</th><th>Gestione</th><th>Valore indicativo</th></tr></thead><tbody>{economicRows.map(row => <tr key={row.user_id}><td>{row.name}</td><td>{row.hours.toLocaleString('it-IT', { useGrouping: 'always',  maximumFractionDigits: 2 })}</td><td>{row.management}</td><td>{row.amount == null ? <span>Da definire<small>Tariffa o maggiorazione mancante/non numerica</small></span> : row.amount.toLocaleString('it-IT', { useGrouping: 'always',  style: 'currency', currency: 'EUR' })}</td></tr>)}</tbody></table></div> : <Empty>Nessuno straordinario approvato nel mese per i dipendenti selezionati.</Empty>}</section>}
