...
export default function HrModule({ configuration = false }) {
  ...
  const calendarActive = page === 'calendar' || page === 'company-calendar';
  ...
  return <div className="hr-page">
    ...
    <div className="hr-controls"><nav className="hr-nav" aria-label="Sezioni HR">{tabs.map(([id, Icon, label]) => <button key={id} className={page === id ? 'active' : ''} aria-current={page === id ? 'page' : undefined} onClick={() => { setPage(id); setFilter(''); if (id === 'calendar') setCalendarView('month'); if (id === 'company-calendar') setCalendarView('week'); }}><span><Icon size={23}/></span>{label}</button>)}</nav>
    ...
    {loading ? <div className="hr-panel">Caricamento HR…</div> : !error && <>
      ...
      {configuration && page === 'company-calendar' && calendarView === 'month' && <CompanyCalendar snapshot={data} month={month} onMonthChange={setMonth} onViewChange={setCalendarView} onReload={async () => { const calendar = await hrRpc('workspace_company_calendar_read'); setData(previous => ({ ...previous, companyCalendar: calendar })); }}/>}
