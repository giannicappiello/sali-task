...
export default function CrmB2BAccount({ account, metrics, related, commercialSnapshot: snapshot, journey, period, warning, canWrite, load, onStatus }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const requests = {
        ...
        tasks: loadAllQueryRows((from, to) => supabase.from('v4_fasi_progetto').select('*').eq('crm_customer_key', key).eq('crm_tipo', 'b2b').order('id').range(from, to)).then(take),
  ...
  return <div className="crm-page b2b-account-page">
    ...
    <WorkspaceTaskDialog open={taskDialog.open} phase={taskDialog.phase} crmType="b2b" initialCustomerKey={key} canManage={canWrite} onClose={() => setTaskDialog({ open: false, phase: null })} onSaved={() => { setRevision(v => v + 1); load(); }}/>
