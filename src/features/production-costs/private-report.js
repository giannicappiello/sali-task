...
export function privateProductionSummary(rows) {
 ...
 const comparable=rows.filter(r=>r.actualTotal!=null&&r.workedOct!=null);
 const excluded=rows.filter(r=>!comparable.includes(r));
 const difference=comparable.length?comparable.reduce((s,r)=>s+r.actualTotal-r.workedOct,0):null;
