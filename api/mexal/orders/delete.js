...
export default async function handler(req, res) {
  ...
  try { const admin = createClient(required("SUPABASE_URL"), required("SUPABASE_SERVICE_ROLE_KEY"), { auth: { persistSession: false } }); await requireAdmin(req, admin); const orderId = String(req.body?.orderId || "").trim();
