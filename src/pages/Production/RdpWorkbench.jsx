...
function DetailPanel({ detail, onClose, onDiagnostics, canDecide, onDecision, onRetry, busy }) {
  ...
  return <div className="rdp-detail-backdrop" role="presentation" onMouseDown={onClose}><section className="rdp-detail" role="dialog" aria-modal="true" aria-label="Dettaglio RdP e OCT" onMouseDown={(event) => event.stopPropagation()}>
    ...
    {detail.request && <div className="rdp-request-meta"><span>Stato {badge(detail.request.workspace_status || detail.request.stato, detail.request.stage === "blocked" ? "red" : "blue")}</span><span>Creata {formatDate(detail.request.created_at, true)}</span><span>Tentativi {detail.request.attempt_count ?? 0}</span><span>Contratto v{detail.request.contract_version || 2}</span>{!detail.request.sent_demand_snapshot_id && !detail.request.last_response ? <button type="button" className="secondary-action" onClick={onRetry} disabled={busy}>{busy ? "Retry in corso…" : "Riprendi invio RdP"}</button> : null}</div>}
