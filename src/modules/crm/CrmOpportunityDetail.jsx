  ...
  return <div className="crm-page">
    ...
    <section className="panel crm-panel crm-operational-section">
      ...
      {operationalActivities.length ? <div className="crm-operational-grid">{operationalActivities.map((item) => <article className="panel crm-operational-card" key={item.activity_id}>
        ...
        <div className="crm-operational-links">{item.project_id ? <Link className="secondary-action" to={`/activities/projects?project=${item.project_id}&returnTo=${encodeURIComponent(pageLocation.pathname + pageLocation.search)}`}>Apri progetto</Link> : null}{item.next_task_id ? <Link className="secondary-action" to={`/activities/tasks?task=${item.next_task_id}&returnTo=${encodeURIComponent(pageLocation.pathname + pageLocation.search)}`}>Apri task</Link> : null}</div>
      ...
      {canWrite ? <form className="crm-operational-form" onSubmit={previewOperationalActivity}>
        ...
        <label><span>Reparto</span><select value={operationalDraft.reparto_id} onChange={(event) => setOperationalDraft({ ...operationalDraft, reparto_id: event.target.value })}><option value="">Reparto dell’opportunità</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select></label>
        <label><span>Responsabile</span><select value={operationalDraft.responsabile_id} onChange={(event) => setOperationalDraft({ ...operationalDraft, responsabile_id: event.target.value })}><option value="">Responsabile dell’opportunità</option>{users.map((item) => <option key={item.id} value={item.id}>{`${item.nome || ""} ${item.cognome || ""}`.trim()}</option>)}</select></label>
