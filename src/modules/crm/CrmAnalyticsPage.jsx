...
export default function CrmAnalyticsPage({ type }) {
  ...
  return <div className="crm-page">
    ...
    <div className="crm-kpi-grid"><Metric label="Conversione" value={metrics.conversion_rate == null ? "—" : `${metrics.conversion_rate}%`} text="Opportunità vinte divise per tutte le opportunità chiuse; calcolo server-side sul periodo e sul perimetro autorizzato." to={period.withPeriod(`${config.basePath}/pipeline`, { view: "list" })} /><Metric label="Valore pipeline" value={formatMoney(metrics.pipeline_value)} text="Somma server-side del valore nominale delle opportunità aperte." to={period.withPeriod(`${config.basePath}/pipeline`, { status: "open", view: "list" })} /><Metric label="Pipeline ponderata" value={formatMoney(metrics.weighted_value)} text="Valore moltiplicato per probabilità, per ogni opportunità aperta." to={period.withPeriod(`${config.basePath}/pipeline`, { status: "open", view: "list" })} /><Metric label="Vinte" value={metrics.won || 0} text={`Opportunità chiuse come vinte; valore finale ${formatMoney(metrics.won_value)}.`} to={period.withPeriod(`${config.basePath}/pipeline`, { outcome: "won", view: "list" })} /><Metric label="Perse" value={metrics.lost || 0} text={`Opportunità chiuse come perse; valore finale ${formatMoney(metrics.lost_value)}.`} to={period.withPeriod(`${config.basePath}/pipeline`, { outcome: "lost", view: "list" })} /><Metric label="Durata ciclo" value={metrics.average_cycle_days == null ? "—" : `${metrics.average_cycle_days} gg`} text="Media dei giorni tra apertura e chiusura delle opportunità nel periodo." to={period.withPeriod(`${config.basePath}/pipeline`, { outcome: "closed", view: "list" })} /></div>
