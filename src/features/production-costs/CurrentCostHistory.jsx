...
export default function CurrentCostHistory({token,settings}) {
 ...
 return <section className="pc-current-history" aria-label="Medie storiche attuali" aria-busy={loading}>
  <div className="pc-toolbar"><h2>Medie storiche attuali <CostInfo title="Medie storiche attuali"><p>Dati MES aggiornati all’apertura e quando cambi i turni aggiuntivi. «Aggiorna medie» acquisisce una nuova lettura. Il turno corrente è escluso; i turni completati senza attività sono inclusi.</p><p>I costi usano la tariffa ora/uomo presente nella scheda, anche se non ancora salvata. Le medie sono consultabili anche quando il criterio scelto usa un altro metodo.</p></CostInfo></h2><button type="button" disabled={loading||!ready} onClick={()=>setRevision(n=>n+1)}><RefreshCw size={16}/>{loading?"Acquisizione…":"Aggiorna medie"}</button></div>
