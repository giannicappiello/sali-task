...
export default function AccessUsers() {
  ...
  const loadData = useCallback(async () => {
    ...
    const results = await Promise.all([
      ...
      supabase.from("reparti").select("id,nome,attivo,workspace_hr").order("nome"),
      ...
      supabase.from("workspace_hr_members").select("user_id,active").eq("active", true),
    ]);
    const firstError = results.find((result) => result.error)?.error;
    if (firstError) setMessage({ type: "error", text: firstError.message });
    const [users, roles, departments, operationalDepartments, agents, integrations, exceptions, aiOverrides, modules, areas, screens, customers, customerLinks, hrMembers] = results.map((result) => result.data || []);
    const hrDepartment = departments.find((department) => department.workspace_hr);
    const userDepartments = [...operationalDepartments, ...(hrDepartment ? hrMembers.map((member) => ({ utente_id: member.user_id, reparto_id: hrDepartment.id })) : [])];
  ...
  async function saveUser() {
    ...
    const { error: accessError } = await supabase.rpc("workspace_save_user_access", {
      target_user_id: userId, target_role_id: form.ruolo_id || null, department_ids: form.reparto_ids,
      personal_exceptions: userExceptions.map(({ ambito, codice, decisione, livello_accesso, motivazione, valida_fino_a }) =>
