...
export async function syncOrderDocuments({ supabase, mexal, origin = "manual" }) {
  ...
  let open = 0; let evicted = 0; let failed = 0; const errors = [];
  ...
  const { data: maintenance } = await supabase.from("mexal_ordini_manutenzione").select("*").eq("id", 1).maybeSingle();
  let cleanup = null;
  ...
  await supabase.from("mexal_sync_runs").update({ status, completed_at: new Date().toISOString(), processed: documents?.length || 0, updated: open + evicted, failed, error_message: errors[0]?.message || null, metadata: { source: origin, aperti: open, evasi: evicted, cleanup, errors } }).eq("id", run.id);
  return { sync_run_id: run.id, processed: documents?.length || 0, aperti: open, evasi: evicted, failed, cleanup };
}
