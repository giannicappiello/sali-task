...
function DecisionDialog({ decision, busy, onClose, onConfirm }) {
  const [reason, setReason] = useState("");
  const [v4Decision, setV4Decision] = useState(decision?.preview?.status === "BLOCKED" ? "WITH_SHORTAGES" : "COMPLETE");
  if (!decision) return null;
  const v3 = ["v3", "v4"].includes(decision.kind);
  return <div className="rdp-dialog-backdrop"><section className="rdp-dialog" role="dialog" aria-modal="true" aria-label="Conferma pianificazione"><header><div><span className="rdp-eyebrow">Decisione operatore</span><h2>Genera ordini di produzione</h2></div><button type="button" onClick={onClose} disabled={busy}><X/></button></header><div className="rdp-no-netting"><ShieldAlert/><div><strong>Impatto produttivo</strong><p>{v3 ? "ProgreMES creerà OdP e impegni FIFO; Workspace registrerà soltanto i fabbisogni acquisto certificati." : `ProgreMES creerà o confermerà la pianificazione per ${decision.articleCode}, quantità ${decision.quantity} ${decision.productionUom || decision.octUom}.`}</p></div></div>{v3 && <><label className="rdp-cancel-reason"><span>Decisione</span><select value={v4Decision} onChange={(event) => setV4Decision(event.target.value)}><option value="COMPLETE">Pianificazione completa</option><option value="WITH_SHORTAGES">Conferma con carenze e genera fabbisogni</option></select></label><label className="rdp-cancel-reason"><span>Motivazione obbligatoria</span><textarea rows={3} minLength={5} maxLength={1000} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Indica il motivo della decisione produttiva…"/></label></>}<div className="rdp-dialog-actions"><button type="button" className="secondary-action" onClick={onClose} disabled={busy}>Annulla</button><button type="button" className="primary-action" onClick={() => onConfirm(reason.trim(), v4Decision)} disabled={busy || (v3 && reason.trim().length < 5)}>{busy ? "Conferma…" : "Conferma e genera OP"}</button></div></section></div>;
}
...
export default function RdpWorkbench() {
  ...
  async function confirmDecision(reason = "", v4Decision = "COMPLETE") { if ((!decision?.proposal?.id && decision?.kind !== "v4") || busy) return; setBusy(true); setError(""); setRdpFailure(null); try { const response = decision.kind === "v4" ? await callWorkbench(accessToken, "workspacemes_v4_confirm", { previewId: decision.preview.id, decision: v4Decision, reason }) : await callWorkbench(accessToken, "progremes_production_confirm", { proposalId: decision.proposal.id }); const productionOrder = decision.kind === "v4" ? response.mes?.productionOrders?.[0] : confirmedProductionOrder(response); if (!productionOrder) throw new Error("ProgreMES ha confermato la decisione senza restituire l’OP generato."); setDecision(null); setResult({ kind: "production_order", status: response.workspaceStatus || response.status || response.mes?.status || "Planned", externalId: detail?.request?.external_id, productionOrder }); setDetail(await callWorkbench(accessToken, "progremes_workbench_detail", { requestId: detail.request.id })); await load(); } catch (e) { setError(e.message); setRdpFailure({ code: e.code || "V4_CONFIRM_FAILED", message: e.message }); } finally { setBusy(false); } }
