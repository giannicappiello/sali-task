...
export default function RdpWorkbench() {
  ...
  async function confirmDecision(decision) { if ((!decision?.proposal?.id && decision?.kind !== "v4") || busy) return; setBusy(true); setError(""); setRdpFailure(null); try { const response = decision.kind === "v4" ? await callWorkbench(accessToken, "workspacemes_v4_confirm", { previewId: decision.preview.id, reason: AUTOMATIC_RDP_CONFIRMATION_REASON }) : await callWorkbench(accessToken, "progremes_production_confirm", { proposalId: decision.proposal.id }); const productionOrder = decision.kind === "v4" ? response.mes?.productionOrders?.[0] : confirmedProductionOrder(response); const forecastOnly = response.mes?.status === "FORECAST" && response.mes?.productionCreated === false; if (!productionOrder && !forecastOnly) throw new Error("ProgreMES ha confermato la decisione senza restituire l’OP generato."); const rdpNumber = detail?.request?.rdp_number; setResult({ kind: forecastOnly ? "forecast" : "production_order", status: response.workspaceStatus || response.status || response.mes?.status || "Confirmed", externalId: detail?.request?.external_id, rdpNumber, productionOrder }); setDetail(null); setPreview(null); setSelected([]); setTab(forecastOnly ? "confirmed" : "scheduling"); await load(); } catch (e) { setError(e.message); setRdpFailure({ phase: "confirm", code: e.code || "V4_CONFIRM_FAILED", message: e.message }); } finally { setBusy(false); } }
