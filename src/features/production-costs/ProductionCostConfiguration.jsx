...
export default function ProductionCostConfiguration(){
 ...
 useEffect(()=>{if(!token)return;let alive=true;action(token,"configuration").then(r=>{if(alive){setVersions(r.configurations);setCanWrite(r.canWrite);setConfigurationLoaded(true);if(r.configurations[0])setSettings({...defaultSettings(),...r.configurations[0].settings});}}).catch(e=>alive&&setError(e.message));return()=>{alive=false;};},[token]);
 useEffect(()=>{
 ...
 <section className="pc-panel"><h2>Turni e calendario <CostInfo title="Calendario di rendicontazione"><p>Il primo turno MES attuale è esteso allo storico, con le chiusure MES sulle rispettive date. I turni Workspace dal secondo in poi sono aggiuntivi: non sovrapporli al primo MES. La durata economica della media storica è di 8 ore.</p><p>Queste impostazioni non spostano il piano APS. Dopo le 17:00 nessun costo automatico fuori turno: lo straordinario va registrato nel consuntivo.</p><p>Con mezzo turno superiore e orario 09:00–16:00: fine alle 17:00 = 1 turno + 1 ora straordinaria; fine alle 11:00 del giorno successivo = 1,5 turni. I criteri FILLING sono indipendenti; il metodo a pezzi non aggiunge una seconda quota per turni o straordinari.</p></CostInfo></h2>
 ...
 <section className="pc-panel"><h2>Prezzi stimati <CostInfo title="Prezzi stimati di vendita"><p>Usati solo in assenza del prezzo netto OCT. Non modificano l’ordine commerciale né le fatture.</p></CostInfo></h2>
