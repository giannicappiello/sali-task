...
import MaterialReallocationPanel from "./MaterialReallocationPanel";
...
export default function RdpWorkbench() {
  ...
  return <div className="production-page rdp-workbench">
    {!customerScoped && canDecide ? <MaterialReallocationPanel key={accessToken} accessToken={accessToken} /> : null}
    ...
    {result && <div className="rdp-success"><CheckCircle2/><div><strong>{result.kind === "cancelled" ? "RdP annullata logicamente" : result.kind === "production_order" ? "RdP andata a buon fine, OP generato." : "RdP ricevuta da ProgreMES"}</strong><p>{result.kind === "production_order" ? `${result.productionOrder?.number || result.productionOrder?.id ? `OP ${result.productionOrder.number || result.productionOrder.id} · ` : ""}pronto per la gestione in MES.` : `${result.rdpNumber ? `RDP${result.rdpNumber}` : "RdP"} · Stato ${result.kind === "cancelled" ? "Annullata" : result.status || "Received"}`}</p>{result.kind === "production_order" && <button type="button" onClick={() => requestProgremesWorkspaceWindow(productionOrderProgremesPath(result))}>Apri OP in nuova finestra Workspace</button>}</div></div>}
