...
import { CheckCircle2, ChevronLeft, ChevronRight, Download, FilePlus2, Info, Link2, RefreshCw, Search, ShoppingCart, X } from "lucide-react";
...
export default function PurchaseRequirements() {
  ...
  async function associateSupplier(group, supplierId) {
    const articles = selectedRows(group).map((row) => ({ articleId: row.articleId, articleCode: row.articleCode }));
    if (await run("ADD_ARTICLE_SUPPLIER", { supplierId: Number(supplierId), articles })) {
      setSelected((current) => { const next = new Set(current); group.rows.forEach((row) => next.delete(row.key)); return next; });
    }
  }
      ...
      <div className="purchase-months">{groups.map((group) => {
        ...
        return <section className="purchase-month-card" id={monthId(group.month)} key={group.key}>
          ...
          <div className="purchase-month-actions">
            ...
            <button type="button" className="secondary-action purchase-associate-action" disabled={!canManage || !supplierId || !selectedForGroup.length || Boolean(busy)} onClick={() => associateSupplier(group, supplierId)} title="Salva in Workspace il fornitore per gli articoli selezionati"><Link2 size={16}/>Associa fornitore<ButtonInfo text="Aggiunge il fornitore selezionato alle associazioni stabili degli articoli. Senza scelta manuale verrà preparato un PF per ogni fornitore associato."/></button>
