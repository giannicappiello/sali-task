...
import LegacyMonthlyCalendar from './LegacyMonthlyCalendar';
...
export default function HrModule({ configuration = false }) {
  ...
  return <div className="hr-page">
    ...
    {loading ? <div className="hr-panel">Caricamento HR…</div> : !error && <>
      ...
      {configuration && page === 'company-calendar' && calendarView === 'month' && <LegacyMonthlyCalendar snapshot={data} month={month} today={today} filter={filter} onFilterChange={setFilter} onWeek={() => setCalendarView('week')} onPrevious={() => { const date = new Date(`${month}-01T12:00:00Z`); date.setUTCMonth(date.getUTCMonth() - 1); setMonth(date.toISOString().slice(0, 7)); }} onToday={() => setMonth(today.slice(0, 7))} onNext={() => { const date = new Date(`${month}-01T12:00:00Z`); date.setUTCMonth(date.getUTCMonth() + 1); setMonth(date.toISOString().slice(0, 7)); }} canManage={canManage} onClosure={closure} onShift={shift} name={name}/>}
