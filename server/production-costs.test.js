...
test("original formula versus confirmed SL separates usage and price with overruns negative",()=>{const r=calc();assert.equal(r.plannedMaterialCost,200);assert.equal(r.actualMaterialCost,330);assert.equal(r.materialVariances[0].usageVariance,-20);assert.equal(r.materialVariances[0].priceVariance,-110);assert.equal(r.actualTotal,500);assert.equal(r.plannedTotal,370);assert.equal(r.totalVariance,-130);});
test("station gain remains separate from production costs and revenue",()=>{const r=calc();assert.equal(r.plannedGain,100);assert.equal(r.actualGain,100);assert.equal(r.actualTotal,500);assert.equal(r.plannedRevenue,null);});
...
test("configured estimated prices do not replace missing real OCT revenue",()=>{const r=calc(sampleEvidence,{...config,settings:{...settings,prices:[{articleCode:"FP092Q",unit:"KG",price:1},{articleCode:"FP092Q",unit:"KG",customerCode:"501.001",price:3}]} });assert.equal(r.plannedRevenue,null);});
...
test("objective is once per work even across several shifts",()=>{const e=structuredClone(sampleEvidence);e.works[0].end="2026-09-09T16:00:00";const c=structuredClone(config);c.settings.machines[0].gainPerWork=250;const r=calc(e,c);assert.ok(r.phases[0].actualTurns>1);assert.equal(r.plannedObjective,250);assert.equal(r.actualGain,250);});
