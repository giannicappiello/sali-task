...
export default function RdpWorkbench({ commercial = false, compact = false, onNavigate, onReturnToPlan }) {
  ...
  if (commercial) return <div className="production-page rdp-commercial-screen">
    ...
    <DetailPanel key={detail?.request?.id || detail?.orders?.map((order) => order.id).join(":") || "none"} detail={detail} onClose={() => setDetail(null)} onDiagnostics={openDiagnostic} canDecide={canDecide} canCancel={canCancel} onDecision={confirmDecision} onCancel={() => setCancelTarget(detail.request)} onV3Preview={createV3Preview} onV3Confirm={() => confirmDecision({ kind: "v4", preview: detail.v3.preview, request: detail.request })} busy={busy}/>
