...
export function compareMexalPayloads(getPayload, postPayload) {
  const comparison = {
    ...
    type_differences: [],
    format_differences: [],
    nomenclature_differences: [],
  };

  function compare(getValue, postValue, path) {
    const getType = valueType(getValue);
    const postType = valueType(postValue);

    if (getType !== postType) {
      comparison.type_differences.push({ path, get_type: getType, post_type: postType });
      return;
    }

    if (getType === "string") {
      const getFormat = stringFormat(getValue);
      const postFormat = stringFormat(postValue);
      if (getFormat !== postFormat) {
        comparison.format_differences.push({ path, get_format: getFormat, post_format: postFormat });
      }
      return;
    }

    if (getType === "array") {
      for (let index = 0; index < Math.min(getValue.length, postValue.length); index += 1) {
        compare(getValue[index], postValue[index], `${path}[${index}]`);
      }
      return;
    }
    ...
    const getKeys = Object.keys(getValue);
    const postKeys = Object.keys(postValue);
    ...
    for (const key of getKeys) {
      if (!(key in postValue)) {
        ...
        compare(getValue[key], postValue[key], jsonPath(path, key));
      }
    }

    for (const key of postKeys) {
      if (!(key in getValue) && !getKeyShapes.has(keyShape(key))) {
        comparison.additional_fields.push({ path: jsonPath(path, key), post_type: valueType(postValue[key]) });
      }
    }
  }
