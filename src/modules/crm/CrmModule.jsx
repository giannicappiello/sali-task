...
const CRM_KPI_INFO = {
  ...
  "Opportunità aperte": "Numero di opportunità commerciali non chiuse nel perimetro corrente.",
  "Valore pipeline": "Somma del valore nominale dei progetti compresi nella pipeline corrente.",
  "Pipeline ponderata": "Somma, per ogni progetto, del valore moltiplicato per la probabilità di chiusura.",
  "Valore ponderato": "Somma, per ogni progetto, del valore moltiplicato per la probabilità di chiusura.",
  "Progetti scaduti": "Progetti aperti con data di chiusura prevista già superata.",
  Scadute: "Progetti aperti con data di chiusura prevista già superata.",
...
const ACCOUNT_COLUMNS = [
  ...
  { value: (row) => formatDate(row.ultimo_ordine_il || row.last_purchase) },
  { value: (row) => row.average_gap_days == null ? "—" : `${Number(row.average_gap_days).toLocaleString("it-IT", { useGrouping: 'always',  maximumFractionDigits: 1 })} gg` },
  { value: (row) => formatDate(row.expected_reorder_date) },
  { value: (row) => row.agente_classificazione || "Senza agente" },
  { value: (row) => `${row.crm_active ? "Attivo" : "Non attivo"} ${row.crm_status_reason || ""}` },
  { value: (row) => row.source === "Workspace/Mexal" ? (row.attivo_mexal === false ? "Annullato" : "Attivo") : "Non presente" },
  { value: (row) => row.crm_active ? "Apri cliente Nuova attività Nuovo progetto Disattiva" : "Apri cliente Nuova attività Nuovo progetto Riattiva" },
];
...
function CrmDashboard({ type }) {
  ...
  return <div className="crm-page">
    ...
    {loading ? <div className="crm-loading">Caricamento KPI...</div> : <div className="crm-kpi-grid">
      ...
      <Kpi label="Progetti scaduti" value={Number(data.overdue_opportunities || 0).toLocaleString("it-IT")} to={period.withPeriod(`${config.basePath}/pipeline`, { overdue: "1" })} />
      ...
      {type === "b2b" ? <Kpi label="Primi ordini da confermare" value={firstOrderSuggestions.length.toLocaleString("it-IT")} note="Rilevati da Mexal; chiusura progetto con conferma" to={period.withPeriod(`${config.basePath}/pipeline`, { view: "list" })} /> : null}
    </div>}
    ...
    {type === "b2b" && firstOrderSuggestions.length ? <section className="panel crm-panel"><h3>Primi ordini rilevati da Mexal</h3><p>Il cliente è già acquisito: verifica l’ordine e conferma la chiusura del progetto dalla scheda completa.</p><div className="crm-list-grid">{firstOrderSuggestions.slice(0, 12).map((item) => <Link className="crm-list-card" key={item.opportunity_id} to={period.withPeriod(`${config.basePath}/pipeline/${item.opportunity_id}`)}><span>Primo ordine · {formatDate(item.first_order_date)}</span><h3>{item.customer_name}</h3><p>{item.opportunity_title}</p><small>{formatMoney(item.first_order_value)} · conferma richiesta</small></Link>)}</div></section> : null}
...
function AccountsPage({ type }) {
  ...
  const { profile, canUseModule, hasPermission } = useAuth();
  ...
  function openCreation(event, kind, customer) {
    event.currentTarget.closest("details").open = false;
    setCreation({ kind, customerKey: customer.entityKey });
  }
  ...
  const load = useCallback(async () => {
    ...
    const [canonicalResult, prospectResult, countryResult, cadenceResult] = await Promise.all([
      ...
      supabase.rpc("crm_customer_cadence_details", { p_crm_type: type, p_to: period.to }),
    ]);
    const loadError = canonicalResult.error || prospectResult.error || countryResult.error || cadenceResult.error;
    if (loadError) { setError(loadError.message); setLoading(false); return; }
    const countries = new Map((countryResult.data || []).map((row) => [row.entity_key, row.country_code]));
    const cadence = new Map((cadenceResult.data || []).map((row) => [row.codice_cliente, row]));
    const canonicalRows = (canonicalResult.data || []).map((row) => ({
      ...row, ...(cadence.get(row.codice_cliente) || {}), entityKey: `mexal:${row.codice_cliente}`, routeId: customerRouteId("mexal", row.codice_cliente),
  ...
  return <div className="crm-page">
    ...
    <ErrorBox error={error} retry={load} />
    {loading ? <div className="crm-loading">Caricamento clienti...</div> : null}<div className="crm-table-wrap"><table ref={tableRef} data-column-controls-mode="dataset" className="crm-table crm-customer-table crm-control-table"><thead><tr><th>Cliente</th><th>Fatturato</th><th>Ordinato</th><th>Ultimo ordine</th><th>Frequenza</th><th>Riordino previsto</th><th>Responsabile</th><th>Stato CRM</th><th>Stato Mexal</th><th>Azioni</th></tr></thead><tbody>{pageRows.map((row) => <tr key={row.entityKey}><td><CrmCustomerLink crmType={type} customerKey={row.entityKey} name={row.nome} period={period}><strong>{row.nome}</strong><small>{row.codice}</small></CrmCustomerLink></td><td><strong>{formatMoney(row.invoice_total)}</strong><small>{row.invoice_count || 0} fatture</small></td><td><strong>{formatMoney(row.order_total)}</strong><small>{row.order_count || 0} ordini</small></td><td>{formatDate(row.ultimo_ordine_il || row.last_purchase)}</td><td>{row.average_gap_days == null ? "—" : `${Number(row.average_gap_days).toLocaleString("it-IT", { useGrouping: 'always',  maximumFractionDigits: 1 })} gg`}</td><td>{formatDate(row.expected_reorder_date)}</td><td>{row.agente_classificazione || "Senza agente"}</td><td><CrmCustomerStatusBadge active={row.crm_active} />{row.crm_status_reason ? <small>{row.crm_status_reason}</small> : null}</td><td>{row.source === "Workspace/Mexal" ? (row.attivo_mexal === false ? "Annullato" : "Attivo") : "Non presente"}</td><td><details className="crm-row-actions"><summary>Azioni</summary><div><CrmCustomerLink crmType={type} customerKey={row.entityKey} name={row.nome} period={period}>Apri cliente</CrmCustomerLink>{canWrite ? <button type="button" onClick={(event) => openCreation(event, "task", row)}>Nuova attività</button> : null}{canWrite && hasPermission("projects.write") ? <button type="button" onClick={(event) => openCreation(event, "project", row)}>Nuovo progetto</button> : null}{canWrite ? <button type="button" className={row.crm_active ? "danger-action" : "secondary-action"} onClick={() => setStatusCustomer(row)}>{row.crm_active ? "Disattiva" : "Riattiva"}</button> : null}</div></details></td></tr>)}</tbody></table>{!loading && !pageRows.length ? <div className="crm-empty">Nessun cliente corrisponde allo stato CRM e ai filtri selezionati.</div> : null}</div>
    ...
    <WorkspaceTaskDialog open={creation?.kind === "task"} crmType={type} initialCustomerKey={creation?.customerKey || ""} canManage={canWrite} onClose={() => setCreation(null)} onSaved={load} />
    <WorkspaceProjectCreateDialog open={creation?.kind === "project"} crmType={type} initialCustomerKey={creation?.customerKey || ""} onClose={() => setCreation(null)} onSaved={load} />
...
function Pipeline({ type, forceList = false }) {
  ...
  const view = forceList || pipelineParams.get("view") === "list" ? "list" : "kanban";
  ...
  async function create(event) {
    ...
    if (!form.customer_ref) { setError("Cerca e seleziona un cliente prima di creare il progetto."); return; }
    try {
      ...
      setForm({ titolo: "", customer_ref: "", valore: "", probabilita: "20", chiusura_prevista: "" }); setCustomerSearch(""); await load();
  ...
  async function move(id, stageId) {
    ...
    if (targetStage?.finale) {
      setError("Le chiusure Vinta/Persa richiedono conferma nella scheda completa del progetto.");
      return;
    }
  ...
  const normalizedCustomerSearch = customerSearch.trim().toLocaleLowerCase("it-IT");
  const customerSuggestions = normalizedCustomerSearch.length < 2 ? [] : [
    ...accounts
      .filter((account) => !account.codice_cliente_mexal && account.nome.toLocaleLowerCase("it-IT").includes(normalizedCustomerSearch))
      .map((account) => ({ key: `crm:${account.id}`, label: account.nome, detail: "Prospect CRM" })),
    ...customers.map((customer) => ({ key: `mexal:${customer.codice_cliente}`, label: customer.ragione_sociale, detail: customer.codice_cliente })),
  ].slice(0, 12);

  function selectCustomer(customer) {
    setForm((current) => ({ ...current, customer_ref: customer.key }));
    setCustomerSearch(`${customer.label} · ${customer.detail}`);
  }
  ...
  return <div className="crm-page">
    ...
    {canWrite ? <form className="crm-card-form crm-project-create-form" onSubmit={create}>
      <label><span>Titolo progetto</span><input required placeholder="Inserisci il titolo" value={form.titolo} onChange={(event) => setForm({ ...form, titolo: event.target.value })} /></label>
      <label className="crm-customer-quick-search"><span>Cliente</span><div className="crm-quick-search-control"><Search size={16} aria-hidden="true" /><input role="combobox" aria-expanded={!form.customer_ref && customerSuggestions.length > 0} aria-controls="crm-customer-suggestions" autoComplete="off" placeholder="Ricerca rapida cliente o codice" value={customerSearch} onChange={(event) => { setCustomerSearch(event.target.value); setCustomers([]); setForm((current) => ({ ...current, customer_ref: "" })); }} /></div>{!form.customer_ref && normalizedCustomerSearch.length >= 2 ? <div id="crm-customer-suggestions" className="crm-quick-search-results" role="listbox">{customerSuggestions.map((customer) => <button type="button" role="option" aria-selected="false" key={customer.key} onClick={() => selectCustomer(customer)}><strong>{customer.label}</strong><span>{customer.detail}</span></button>)}{!customerSuggestions.length ? <small>Nessun cliente trovato.</small> : null}</div> : null}</label>
      <label><span>Valore</span><input type="number" min="0" step="0.01" placeholder="0,00" value={form.valore} onChange={(event) => setForm({ ...form, valore: event.target.value })} /></label>
      <label><span>Probabilità</span><input type="number" min="0" max="100" placeholder="%" value={form.probabilita} onChange={(event) => setForm({ ...form, probabilita: event.target.value })} /></label>
      <label><span>Chiusura prevista</span><input type="date" value={form.chiusura_prevista} onChange={(event) => setForm({ ...form, chiusura_prevista: event.target.value })} /></label>
      <button className="primary-action crm-primary"><Plus size={16} />Crea opportunità</button>
    </form> : null}
    <div className="crm-filters crm-pipeline-filters"><label><Search size={16} /><input placeholder="Cerca progetto o cliente" value={search} onChange={(event) => updatePipelineParam("search", event.target.value)} /></label><select value={stageFilter} onChange={(event) => updatePipelineParam("stage", event.target.value)}><option value="">Tutte le fasi</option>{stages.map((stage) => <option key={stage.id} value={stage.id}>{stage.nome}</option>)}</select><select value={pipelineParams.get("nextStep") || "all"} onChange={(event) => updatePipelineParam("nextStep", event.target.value === "all" ? "" : event.target.value)}><option value="all">Tutti i prossimi passi</option><option value="overdue">Prossimo passo scaduto</option><option value="upcoming">Prossimo passo futuro</option><option value="missing">Prossimo passo mancante</option></select><CrmCustomerStatusFilter id={`crm-pipeline-status-${type}`} value={customerStatus} onChange={setCustomerStatus} />{!forceList ? <div className="crm-view-toggle"><button type="button" className={view === "kanban" ? "active" : ""} onClick={() => updatePipelineParam("view", "kanban")}>Kanban</button><button type="button" className={view === "list" ? "active" : ""} onClick={() => updatePipelineParam("view", "list")}>Lista</button></div> : null}</div>
    {loading ? <div className="crm-loading">Caricamento pipeline...</div> : !visibleItems.length ? <div className="crm-empty"><strong>Nessun progetto nel perimetro corrente.</strong><p>Crea il primo progetto da un cliente Workspace/Mexal o da un prospect CRM-only.</p></div> : view === "kanban" ? <div className="crm-kanban">{stages.filter((stage) => !stageFilter || stage.id === stageFilter).map((stage) => <section key={stage.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => void move(event.dataTransfer.getData("text/plain"), stage.id)}><header><strong>{stage.nome}</strong><span>{visibleItems.filter((item) => item.stage_id === stage.id).length}</span></header>{visibleItems.filter((item) => item.stage_id === stage.id).map((item) => <article draggable={canWrite} onDragStart={(event) => event.dataTransfer.setData("text/plain", item.id)} key={item.id}><Link className="crm-opportunity-link" to={period.withPeriod(`${config.basePath}/pipeline/${item.id}`)}>{item.titolo}</Link><CrmCustomerLink crmType={type} customerCode={item.crm_accounts?.codice_cliente_mexal} accountId={item.crm_accounts?.id} name={item.crm_accounts?.nome} period={period}>{item.crm_accounts?.nome}</CrmCustomerLink><small>{formatMoney(item.valore)} · {item.probabilita || 0}% · {stageAge(item)} gg nello stato</small>{nextActivity(item) ? <small>Prossima: {formatDate(nextActivity(item).data_attivita)}</small> : <small className="crm-missing-step">Nessuna prossima attività</small>}<CrmDeleteProjectButton project={item} canDelete={canWrite} onDeleted={load} onError={setError} compact /></article>)}</section>)}</div> : <div className="crm-table-wrap"><table className="crm-table"><thead><tr><th>Progetto</th><th>Cliente</th><th>Fase</th><th>Valore</th><th>Ponderato</th><th>Giorni fase</th><th>Chiusura</th><th>Prossima attività</th><th>Azioni</th></tr></thead><tbody>{visibleItems.map((item) => <tr key={item.id}><td><Link className="crm-table-link" to={period.withPeriod(`${config.basePath}/pipeline/${item.id}`)}>{item.titolo}</Link></td><td><CrmCustomerLink crmType={type} customerCode={item.crm_accounts?.codice_cliente_mexal} accountId={item.crm_accounts?.id} name={item.crm_accounts?.nome} period={period}>{item.crm_accounts?.nome}</CrmCustomerLink></td><td>{item.crm_opportunity_stages?.nome}</td><td>{formatMoney(item.valore)}</td><td>{formatMoney(Number(item.valore || 0) * Number(item.probabilita || 0) / 100)}</td><td>{stageAge(item)}</td><td>{formatDate(item.chiusura_prevista)}</td><td>{nextActivity(item)?.titolo || <span className="crm-missing-step">Mancante</span>}</td><td><CrmDeleteProjectButton project={item} canDelete={canWrite} onDeleted={load} onError={setError} compact /></td></tr>)}</tbody></table></div>}
  </div>;
}
...
function BriefsPage() {
  ...
  return <div className="crm-page">
    ...
    {canWrite ? <form className="crm-card-form panel crm-form-grid" onSubmit={save}>
      ...
      <select value={form.opportunity_id} onChange={(event) => setField("opportunity_id", event.target.value)}><option value="">Progetto commerciale opzionale</option>{opportunities.filter((item) => !form.account_id || item.account_id === form.account_id).map((item) => <option key={item.id} value={item.id}>{item.titolo}</option>)}</select>
...
function renderCrmView(route) {
  switch (route.view) {
    ...
    case "customer-products": return <CustomerProductsPage kind={route.kind} />;
    case "pipeline": return <Pipeline type={route.type} />;
    case "opportunities": return <Pipeline type={route.type} forceList />;
