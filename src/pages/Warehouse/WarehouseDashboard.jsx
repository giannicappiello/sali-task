    ...
    <div className="warehouse-dashboard-page">
      ...
      {!loading && !error && data.snapshotAvailable ? <>
        ...
        <section className="warehouse-dashboard-kpis">
          ...
          <article><span>Articoli con giacenza negativa<InfoTip text="Numero di codici articolo distinti con almeno una giacenza negativa nel dataset filtrato. Restano visibili nell’elenco." label="Articoli con giacenza negativa" /></span><strong>{quantityFormat.format(summary.negativeArticles || 0)}</strong><small>Visibili nell’elenco</small></article>
          {!customerScoped && <article><span>Articoli senza costo<InfoTip text="Numero di codici articolo distinti con costo ultimo Mexal assente o uguale a zero nel dataset filtrato." label="Articoli senza costo" /></span><strong>{quantityFormat.format(summary.unvaluedArticles || 0)}</strong><small>Non valorizzabili economicamente</small></article>}
        </section>
