...
export default function PrivateDocuments() {
  ...
  const loadSearch = useCallback(async (search) => {
    ...
    try { const active = documentSessionRef.current || await establishSession(); const result = await documentRequest(active, `articles?search=${encodeURIComponent(search.trim())}`) || []; if (sequence === searchSequence.current) setArticles(result); }
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      const active = documentSessionRef.current || await establishSession();
      const completeCatalog = (await documentRequest(active, "articles"))
        .sort((left, right) => String(left.articleCode).localeCompare(String(right.articleCode), "it", { numeric: true }));      setCatalog(completeCatalog);
  ...
  async function upload(event) {
    ...
    try {
      ...
      setUploadOpen(false);
      await workspaceAction(accessToken, "private_documents_sync");
  ...
  return <div className="private-documents-page">
    ...
    {syncInfo && <small className="private-documents-sync">Archivio Workspace aggiornato: {syncInfo.documents} documenti · {syncInfo.slRows} righe SL</small>}
