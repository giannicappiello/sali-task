...
const searchText = (row) => [row.articleCode, row.description, typeLabel(row.articleType), row.requiredAt, row.orderBy,
  row.unitOfMeasure, row.pfDocuments, row.supplierOrders, row.supplierName,
  ...(row.workspaceSuppliers || []).flatMap((supplier) => [supplier.ragioneSociale, supplier.codiceMexal]), row.octReferences,
...
export default function PurchaseRequirements() {
  ...
  function toggleGroup(group) {
    const ids = group.rows.filter((row) => row.quantityToOrder > 0 && !row.requiresReview).map((row) => row.key);
  ...
  const selectedRows = (group) => group.rows.filter((row) => row.quantityToOrder > 0 && !row.requiresReview && selected.has(row.key));
  ...
  return <div className="production-page purchase-requirements-page">
    ...
    {data.warnings?.length > 0 && <div className="purchase-feedback" role="status"><div><strong>Calcolo completato con anomalie da verificare</strong><p>I materiali segnalati sono esclusi dalle proposte PF; gli altri restano elaborabili.</p><ul>{data.warnings.map((warning, index) => <li key={index}>{warning}</li>)}</ul></div></div>}
    ...
    {loading ? <div className="production-loading">Calcolo dei fabbisogni in corso…</div> : <>
      ...
      <div className="purchase-months">{groups.map((group) => {
        ...
        const groupToOrder = visibleRows.filter((row) => row.quantityToOrder > 0 && !row.requiresReview);
        ...
        return <section className="purchase-month-card" id={monthId(group.month)} key={group.key}>
          ...
          <div className="purchase-month-actions">
            ...
            <button type="button" className="primary-action" disabled={!canManage || !selectedForGroup.length || Boolean(busy)} onClick={() => preparePfPreview({ mode: "selected", selectedKeys: selectedForGroup.map((row) => row.key), supplierId: supplierId ? Number(supplierId) : null })} title="Genera l’anteprima dei PF per i soli articoli selezionati"><FilePlus2 size={16}/>{busy === "PREVIEW_PF" ? "Anteprima…" : "Genera PF da selezionati"}<ButtonInfo text="Prepara i PF dei soli articoli selezionati. Senza scelta manuale usa automaticamente tutti i fornitori ricavati dallo storico ordini Mexal."/></button>
            <button type="button" className="secondary-action" disabled={!canManage || Boolean(busy) || !toOrder.length} onClick={() => preparePfPreview({ mode: "automatic" })} title="Genera l’anteprima dei PF necessari entro 60 giorni da oggi"><FilePlus2 size={16}/>Genera PF automatico<ButtonInfo text="Prepara automaticamente un PF per ogni fornitore storico associato agli articoli necessari entro 60 giorni, evitando duplicati; l’emissione richiede conferma."/></button>
          </div>
          <div className="purchase-table-wrap" data-column-controls="off"><table><colgroup><col className="purchase-select-column"/><col className="purchase-material-column"/><col className="purchase-type-column"/><col className="purchase-required-column"/><col className="purchase-order-by-column"/><col className="purchase-demand-column"/><col className="purchase-stock-column"/><col className="purchase-arrival-column"/><col className="purchase-order-column"/><col className="purchase-pf-column"/><col className="purchase-supplier-column"/><col className="purchase-oct-column"/><col className="purchase-production-column"/><col className="purchase-status-column"/></colgroup><thead><tr><th className="purchase-select-cell" aria-label="Seleziona"></th><th>Materiale</th><th>Tipo</th><th>Necessario entro</th><th>Ordina entro</th><th>Fabbisogno</th><th>Giacenza</th><th>In arrivo</th><th>Da ordinare</th><th>PF Mexal</th><th>Consegne / fornitori</th><th>OCT</th><th>Ordini produzione</th><th>Stato</th></tr></thead><tbody>{visibleRows.map((row) => <tr key={row.key} className={row.quantityToOrder > 0 ? "to-order" : ""}><td className="purchase-select-cell">{row.quantityToOrder > 0 && !row.requiresReview && <input type="checkbox" checked={selected.has(row.key)} onChange={() => toggle(row)} aria-label={`Seleziona ${row.articleCode}`}/>}</td><td className="purchase-material-cell"><strong>{row.articleCode}</strong><small>{row.description}</small></td><td><span className="purchase-type">{typeLabel(row.articleType)}</span></td><td>{date(row.requiredAt)}</td><td><strong className={row.status === "ORDER_LATE" ? "late" : ""}>{date(row.orderBy)}</strong><small>{row.leadTimeDays} gg</small></td><td className="number">{quantity(row.requiredQuantity)} {row.unitOfMeasure}</td><td className="number">{quantity(row.availableStock)}</td><td className="number">{quantity(row.incomingQuantity)}</td><td className="number"><strong className={row.quantityToOrder > 0 ? "late" : "covered"}>{quantity(row.quantityToOrder)}</strong>{row.reorderLot > 0 && row.quantityToOrder > 0 && <small>netto {quantity(row.netRequirement)} · lotto {quantity(row.reorderLot)}</small>}</td><td>{row.pfDocuments ? <><span className="purchase-pf">{row.pfDocuments}</span><small>{quantity(row.pfQuantity)} {row.unitOfMeasure} proposti</small></> : "—"}</td><td><span>{row.supplierOrders || "Nessuna consegna datata"}</span>{row.supplierName && <small>Priorità MES: {row.supplierName}</small>}<div className="purchase-supplier-links">{(row.workspaceSuppliers || []).map((supplier) => <span key={supplier.associationId} title={`Associazione automatica Workspace · ${supplier.codiceMexal || "senza codice"}`}>{supplier.ragioneSociale}</span>)}</div></td><td className="purchase-lineage">{row.octReferences || "—"}</td><td className="purchase-lineage">{row.productionOrders || "—"}</td><td><span className={`purchase-status ${row.status.toLowerCase()}`}>{row.requiresReview ? "Da verificare · escluso dai PF" : statusLabel(row.status)}</span></td></tr>)}</tbody></table>{!visibleRows.length && <p className="table-message">Nessun fabbisogno corrisponde alla ricerca.</p>}</div>
        </section>;
      })}</div>
    ...
    {pfPreview && <div className="pf-preview-backdrop" role="presentation"><section className="pf-preview-dialog" role="dialog" aria-modal="true" aria-labelledby="pf-preview-title"><header><div><span>Anteprima non emessa</span><h2 id="pf-preview-title">PF Mexal</h2><p>{pfPreview.documents[previewIndex]?.supplierName} · {pfPreview.documents[previewIndex]?.lines.length} articoli</p></div><strong className="pf-preview-counter" aria-live="polite">PF {previewIndex + 1} di {pfPreview.files.length}</strong><button type="button" className="secondary-action" onClick={closePfPreview} disabled={Boolean(busy)} aria-label="Chiudi anteprima"><X size={18}/></button></header><div className="pf-preview-document"><iframe src={pfPreview.files[previewIndex]?.url} title={`Anteprima ${previewIndex + 1} di ${pfPreview.files.length}: ${pfPreview.files[previewIndex]?.name}`}/></div><footer><div className="pf-preview-navigation"><button type="button" className="secondary-action" disabled={previewIndex === 0 || Boolean(busy)} onClick={() => setPreviewIndex((value) => value - 1)}><ChevronLeft size={16}/>Precedente</button><a className="secondary-action" href={pfPreview.files[previewIndex]?.url} download={pfPreview.files[previewIndex]?.name}><Download size={16}/>Scarica anteprima</a><button type="button" className="secondary-action" disabled={previewIndex >= pfPreview.files.length - 1 || Boolean(busy)} onClick={() => setPreviewIndex((value) => value + 1)}>Successivo<ChevronRight size={16}/></button></div><div className="pf-preview-confirm"><span>{pfPreview.documentCount} PF · {pfPreview.lineCount} righe. Nessun documento è stato ancora scritto.{pfPreview.skippedWithoutSupplier > 0 ? ` ${pfPreview.skippedWithoutSupplier} materiali esclusi perché senza fornitore associato.` : ""}</span><button type="button" className="secondary-action" onClick={closePfPreview} disabled={Boolean(busy)}>Annulla</button><button type="button" className="primary-action" onClick={confirmPfPreview} disabled={Boolean(busy)}><FilePlus2 size={16}/>{busy === "CONFIRM_PF_PREVIEW" ? "Emissione…" : "Conferma ed emetti PF"}</button></div></footer></section></div>}
