...
export default function PackagingSheet({ sheet }) {
  return <article className="packaging-sheet">
    ...
    <section><h3>Confezionamento e imballo</h3><div className="packaging-sheet-grid">{['Pezzi per imballo', 'Cartoni per strato', 'Strati per pallet', 'Pezzi per pallet', 'Numero pallet', 'Totale pezzi'].map(label => <p key={label}>{label}: ________________</p>)}</div></section>
