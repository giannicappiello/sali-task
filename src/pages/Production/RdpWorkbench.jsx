...
export default function RdpWorkbench() {
  ...
  async function createV3Preview() { if (!detail?.request?.id || busy) return; setBusy(true); setError(""); setRdpFailure(null); try { const response = await callWorkbench(accessToken, "workspacemes_v3_preview", { requestId: detail.request.id }); const refreshedDetail = await callWorkbench(accessToken, "progremes_workbench_detail", { requestId: detail.request.id }); setDetail(refreshedDetail); const failure = v3RecalculationOutcomeFailure(response, refreshedDetail); if (failure) setRdpFailure(failure); } catch (e) { setError(e.message); setRdpFailure({ code: e.code || "V3_PREVIEW_FAILED", message: e.message }); } finally { setBusy(false); } }
