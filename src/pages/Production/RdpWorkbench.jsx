...
export default function RdpWorkbench({ commercial = false, compact = false, onNavigate, onReturnToPlan }) {
  ...
  async function openDetail(row) { setError(""); try { const loaded = await callWorkbench(accessToken, "progremes_workbench_detail", { orderId: row.orderId || row.id, requestId: row.requestId }); setDetail(loaded); if (!commercial && canDecide && loaded.v3?.preview?.status === "READY" && loaded.v3.preview.snapshot?.confirmationRecovery && !loaded.v3.preview.snapshot.confirmationRecovery.rejected && !loaded.v3.preview.snapshot.confirmationRecovery.reconciliationRequired && !loaded.v3.saga) await confirmDecision({ kind: "v4", preview: loaded.v3.preview, request: loaded.request }); } catch (e) { setError(e.message); } }
  ...
  if (commercial) return <div className="production-page rdp-commercial-screen">
    <CommercialOrdersScreen data={data} stage={tab} onStage={setTab} loading={loading} busy={busy}
      ...
      loadBatches={(row, productionOrderId) => callWorkbench(accessToken, "progremes_workbench_detail", { orderId: row.orderId || row.id, requestId: row.requestId, productionOrderId })}>
