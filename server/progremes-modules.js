...
export async function listProgremesIntegration(req, supabase) {
  ...
  await ensureProgremesCatalogFresh(supabase);
...
export async function syncProgremesModules(req, supabase, origin = "manuale", skipPermissionCheck = false) {
  ...
  const { data: running } = await supabase.from("progremes_sync_runs").select("id").in("stato", ["in_coda", "in_esecuzione"]).limit(1).maybeSingle();
  ...
  try {
    ...
    const { data: runState } = await supabase.from("progremes_sync_runs").select("arresto_richiesto").eq("id", run.id).single();
    if (runState?.arresto_richiesto) {
      const stoppedAt = new Date().toISOString();
      await supabase.from("progremes_sync_runs").update({ stato: "arrestata", completata_il: stoppedAt }).eq("id", run.id);
    ...
    await supabase.from("progremes_sync_runs").update({ stato: "completata", completata_il: now, moduli_letti: normalized.length, inseriti: inserted, aggiornati: updated, disattivati: removed.length, dettagli: { source: payload.source, version: payload.version, screens_disabled: removedScreens.length } }).eq("id", run.id);
    ...
    await supabase.from("progremes_sync_runs").update({ stato: "errore", completata_il: new Date().toISOString(), errore: error.message }).eq("id", run.id);
    throw error;
  }
}
...
export async function stopProgremesModulesSync(req, supabase) {
  await requirePermission(req, supabase, ["integrations.configure", "integrations.sync.progremes_modules"]);
  const { data: run, error } = await supabase.from("progremes_sync_runs").select("id").in("stato", ["in_coda", "in_esecuzione"]).order("iniziata_il", { ascending: false }).limit(1).maybeSingle();
  if (error) throw error;
  if (!run) return { stopped: false };
  const { error: updateError } = await supabase.from("progremes_sync_runs").update({ arresto_richiesto: true }).eq("id", run.id);
