...
import CommercialOrdersScreen from "./CommercialOrdersScreen";
...
async function callWorkbench(accessToken, action, extra = {}) {
  ...
  if (!response.ok || payload.success === false) throw Object.assign(new Error(payload.error || "Operazione Workbench non riuscita."), { code: payload.code || ([502, 503, 504].includes(response.status) ? `PROGREMES_HTTP_${response.status}` : undefined) });
  return payload;
}
...
export function OctOrderCard({ row, selectable, selected, onToggle, onOpen, onDiagnostic, loadBatches }) {
  ...
  return <article className={`rdp-oct-card ${!row.ready && row.stage !== "history" ? "blocked" : ""}`}>
    ...
    {loadBatches && row.productionOrders?.length > 0 && <CommercialBatchProgress orders={row.productionOrders} load={loadBatches}/>}
...
export function DiagnosticActionDialog({ diagnostic, busy, canManage, onClose, onApply }) {
...
export function DetailPanel({ readOnly = false, detail, onClose, onDiagnostics, canDecide, canCancel, onDecision, onCancel, onV3Preview, onV3Confirm, busy }) {
  ...
  return <div className="rdp-detail-backdrop" role="presentation" onMouseDown={onClose}><section className="rdp-detail" role="dialog" aria-modal="true" aria-label="Dettaglio RdP e OCT" onMouseDown={(event) => event.stopPropagation()}>
    ...
    <div className="rdp-line-list">{detail.lines.map((line) => { const hasFinishedBom = Boolean(line.finishedBom?.components?.length); return <article key={line.id} className={line.descriptive ? "rdp-line descriptive" : "rdp-line"}>
      <button type="button" className="rdp-line-summary" onClick={() => setOpenLine(openLine === line.id ? null : line.id)}>
        ...
        {line.descriptive ? badge("OK · NON PRODUTTIVA", "green") : hasFinishedBom ? badge(hasV3Preview ? "V4 · DISTINTA MES" : "DISTINTA DISPONIBILE", "green") : badge(line.mesStatus || line.mappingStatus, blocking(line.diagnostics) ? "red" : "neutral")}<ChevronRight size={18} />
      </button>
    ...
    {detail.request && <V3Panel readOnly={readOnly} v3={detail.v3} canDecide={!readOnly && canDecide} busy={busy} onPreview={onV3Preview} onConfirm={onV3Confirm}/>}
  </section></div>;
}
...
function CancelRequestDialog({ request, busy, onClose, onConfirm }) {
  ...
  return <div className="rdp-dialog-backdrop" role="presentation"><section className="rdp-dialog rdp-cancel-dialog" role="dialog" aria-modal="true" aria-label="Conferma annullamento RdP">
    ...
    <div className="rdp-cancel-warning"><ShieldAlert/><div><strong>{rdpLabel(request)}</strong><p>MES verificherà anche le RdP confermate: annullerà le previsioni e gli OP non avviati e libererà i relativi impegni. Lavorazioni avviate, consumi e documenti SL/CL restano protetti. Solo dopo la conferma di MES lo stato diventerà Annullata.</p><p>OCT, lotti già attribuiti, diagnostica e storico saranno conservati. L’OCT, se ancora presente e con residuo disponibile, tornerà selezionabile; nessuna nuova RdP sarà creata automaticamente. Gli acquisti già documentati non saranno annullati: andranno verificati separatamente.</p></div></div>
...
export default function RdpWorkbench({ commercial = false, compact = false, onNavigate, onReturnToPlan }) {
  ...
  const availableTabs = compact ? TABS.filter(([code]) => !["mes-orders", "planning"].includes(code)) : commercial ? [["all", "Tutti gli ordini"], ...TABS.filter(([code]) => !["purchasing", "mes-orders", "planning"].includes(code))] : customerScoped ? TABS.filter(([code]) => !["purchasing", "mes-orders", "planning"].includes(code)) : TABS;
  ...
  async function load() { setLoading(true); setError(""); try { const payload = await callWorkbench(accessToken, "progremes_workbench_list", { workspaceScreen: commercial ? "workspace.production.progress" : undefined }); setData(workbenchRows(payload)); setProductionGates(payload.productionGates || null); } catch (e) { setProductionGates(null); setError(e.message); } finally { setLoading(false); } }
  useEffect(() => {
    ...
    async function initialize() {
      ...
      setError("");
      try {
        const initial = await callWorkbench(accessToken, "progremes_workbench_list", { workspaceScreen: commercial ? "workspace.production.progress" : undefined });
  ...
  }, [accessToken, customerScoped, commercial]);
    ...
    async function poll() {
      try {
        ...
        if (payload.refresh?.status === "completed") {
          const refreshed = await callWorkbench(accessToken, "progremes_workbench_list", { workspaceScreen: commercial ? "workspace.production.progress" : undefined });
  ...
  }, [accessToken, octJobId, octStatus, commercial]);
  ...
  const selectionBlocked = selectedRows.length !== selected.length || selectedRows.some((row) => !row.ready || blocking(row.diagnostics) || (commercial && row.stage !== "evaluation"));
  ...
  async function openDetail(row) { if (commercial && customerScoped) return; setError(""); try { const loaded = await callWorkbench(accessToken, "progremes_workbench_detail", { workspaceScreen: commercial ? "workspace.production.progress" : undefined, orderId: row.orderId || row.id, requestId: row.requestId }); setDetail(loaded); if (!commercial && canDecide && loaded.v3?.preview?.status === "READY" && loaded.v3.preview.snapshot?.confirmationRecovery && !loaded.v3.preview.snapshot.confirmationRecovery.rejected && !loaded.v3.preview.snapshot.confirmationRecovery.reconciliationRequired && !loaded.v3.saga) await confirmDecision({ kind: "v4", preview: loaded.v3.preview, request: loaded.request }); } catch (e) { setError(e.message); } }
  async function createPreview() { if (!canCreate || !sendEnabled || !selected.length || selectionBlocked || busy) return; setBusy(true); setError(""); try { setPreview(await callWorkbench(accessToken, "workspacemes_v4_precheck", { orderIds: selected })); } catch (e) { setError(e.message); } finally { setBusy(false); } }
  async function sendRequest() { if (!sendEnabled || !preview || busy) return; setBusy(true); setError(""); setRdpFailure(null); try { const response = await callWorkbench(accessToken, "workspacemes_v4_request", { orderIds: selected, snapshotId: preview.snapshot.id }); if (!response.requestId) throw new Error("Workspace non ha restituito il riferimento RdP V4."); setPreview(null); setSelected([]); await load(); const outcome = await callWorkbench(accessToken, "progremes_workbench_detail", { workspaceScreen: commercial ? "workspace.production.progress" : undefined, requestId: response.requestId }); setResult({ ...response, rdpNumber: outcome.request?.rdp_number }); setDetail(outcome); if (response.previewError) { const failure = { code: response.previewError.code || "V4_PREVIEW_FAILED", message: response.previewError.message }; setError(`RdP V4 creata ma non elaborata: ${failure.message}`); setRdpFailure(failure); } } catch (e) { const message = e.code === "DEMAND_CHANGED" ? "L’OCT è cambiato dopo l’anteprima: ripetere il precheck." : e.code === "RDP_IDEMPOTENCY_CONFLICT" ? "La selezione appartiene a una precedente generazione RdP. Aggiorna la schermata e crea una nuova anteprima." : e.message; setError(message); setRdpFailure({ code: e.code || "V4_REQUEST_FAILED", message }); } finally { setBusy(false); } }
  async function createV3Preview() { if (!detail?.request?.id || busy) return; setBusy(true); setError(""); setRdpFailure(null); try { await callWorkbench(accessToken, "workspacemes_v4_preview", { requestId: detail.request.id }); const refreshedDetail = await callWorkbench(accessToken, "progremes_workbench_detail", { workspaceScreen: commercial ? "workspace.production.progress" : undefined, requestId: detail.request.id }); setDetail(refreshedDetail); } catch (e) { setError(e.message); setRdpFailure({ code: e.code || "V4_PREVIEW_FAILED", message: e.message }); } finally { setBusy(false); } }
  async function confirmDecision(decision) { if ((!decision?.proposal?.id && decision?.kind !== "v4") || busy) return; setBusy(true); setError(""); setRdpFailure(null); try { const response = decision.kind === "v4" ? await recoverConfirmation(() => callWorkbench(accessToken, "workspacemes_v4_confirm", { previewId: decision.preview.id, reason: AUTOMATIC_RDP_CONFIRMATION_REASON }), { onPending: () => setError("Conferma in verifica: recupero automatico esito MES...") }) : await callWorkbench(accessToken, "progremes_production_confirm", { proposalId: decision.proposal.id }); const productionOrder = decision.kind === "v4" ? response.mes?.productionOrders?.[0] : confirmedProductionOrder(response); const forecastOnly = response.mes?.status === "FORECAST" && response.mes?.productionCreated === false; if (!productionOrder && !forecastOnly) throw new Error("ProgreMES ha confermato la decisione senza restituire l’OP generato."); const rdpNumber = decision.request?.rdp_number || detail?.request?.rdp_number; setResult({ kind: forecastOnly ? "forecast" : "production_order", status: response.workspaceStatus || response.status || response.mes?.status || "Confirmed", externalId: decision.request?.external_id || detail?.request?.external_id, rdpNumber, productionOrder }); setError(""); setDetail(null); setPreview(null); setSelected([]); setTab(forecastOnly ? "confirmed" : "scheduling"); await load(); } catch (e) { setError(e.message); setRdpFailure({ phase: "confirm", code: e.code || "V4_CONFIRM_FAILED", message: e.message }); } finally { setBusy(false); } }
  ...
  async function applyDiagnosticAction(row, diagnosticAction, reason) {
    ...
    try {
      ...
      if (requestId) setDetail(await callWorkbench(accessToken, "progremes_workbench_detail", { workspaceScreen: commercial ? "workspace.production.progress" : undefined, requestId }));
      await load();
  ...
  if (commercial) return <div className="production-page rdp-commercial-screen">
    <CommercialOrdersScreen data={data} stage={tab} onStage={setTab} loading={loading} busy={busy}
      onReload={load} onSync={refreshOctOrders} syncRunning={octRefreshRunning} customerScoped={customerScoped}
      syncStatus={octRefresh ? <BackgroundSyncStatus refresh={octRefresh}/> : null}
      selected={selected} onToggle={toggle} onClearSelection={() => { setSelected([]); setPreview(null); }}
      canCreate={canCreate} sendEnabled={sendEnabled} selectionBlocked={selectionBlocked} onPreview={createPreview}
      onOpen={openDetail} onDiagnostic={openDiagnostic} onBack={() => navigate("/produzione")}
      loadBatches={(row, productionOrderId) => callWorkbench(accessToken, "progremes_workbench_detail", { workspaceScreen: commercial ? "workspace.production.progress" : undefined, orderId: row.orderId || row.id, requestId: row.requestId, productionOrderId })}>
    ...
    {result && <div className="rdp-success"><CheckCircle2/><div><strong>{result.kind === "cancelled" ? "RdP annullata logicamente" : result.kind === "forecast" ? "RdP confermata: previsione di capacità creata, senza OP, impegni o lotti." : result.kind === "production_order" ? "RdP andata a buon fine, OP generato." : "RdP ricevuta da ProgreMES"}</strong><p>{result.kind === "production_order" ? `${result.productionOrder?.number || result.productionOrder?.id ? `OP ${result.productionOrder.number || result.productionOrder.id} · ` : ""}pronto per la gestione in MES.` : `${result.rdpNumber ? `RDP${result.rdpNumber}` : "RdP"} · Stato ${result.kind === "cancelled" ? "Annullata" : result.status || "Received"}`}</p>{result.kind === "forecast" && <button type="button" className="rdp-forecast-action" onClick={() => compact ? onReturnToPlan?.() : requestProgremesWorkspaceWindow("/produzione/progremes.PlanningProduction")}><span>Apri previsione e conferma piano</span><ChevronRight size={18} aria-hidden="true" /></button>}{result.kind === "production_order" && <button type="button" onClick={() => compact ? onReturnToPlan?.() : requestProgremesWorkspaceWindow(productionOrderProgremesPath(result))}>{compact ? "Torna al piano aggiornato" : "Apri OP in nuova finestra Workspace"}</button>}</div></div>}
    </CommercialOrdersScreen>
    <DetailPanel key={detail?.request?.id || detail?.orders?.map((order) => order.id).join(":") || "none"} detail={commercial && customerScoped ? null : detail} onClose={() => setDetail(null)} onDiagnostics={openDiagnostic} canDecide={canDecide} canCancel={canCancel} onDecision={confirmDecision} onCancel={() => setCancelTarget(detail.request)} onV3Preview={createV3Preview} onV3Confirm={() => confirmDecision({ kind: "v4", preview: detail.v3.preview, request: detail.request })} busy={busy}/>
