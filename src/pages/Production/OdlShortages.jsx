...
export default function OdlShortages({ rows = [], busy, onOpen }) {
  ...
  return <section className="plan-panel"><h2>Fabbisogni specifici degli ODL</h2>
    ...
    <div className="plan-table-wrap"><table><thead><tr><th>RdP / OP</th><th>Fase</th><th>Articolo</th><th>Quantità</th><th>Necessario il</th><th>Stato</th><th>Azione</th></tr></thead><tbody>{rows.map((row, i) => <tr key={`${row.versionId}:${i}`}><td>{row.orderNumber || row.orderId}</td><td>{phases[row.phase] || row.phase}</td><td>{row.code}</td><td>{new Intl.NumberFormat("it-IT", { useGrouping: 'always',  maximumFractionDigits: 6 }).format(row.quantity)} {row.unit}</td><td>{planningDate(row.requiredAt)}</td><td>{row.cancelled ? "ODL annullato" : row.coveredAtUtc ? "Coperto" : "Da coprire"}</td><td><button disabled={busy} onClick={() => onOpen(row.versionId)}>Apri fabbisogno</button></td></tr>)}</tbody></table></div>
  </section>;
}
