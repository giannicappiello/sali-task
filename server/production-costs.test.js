...
const settings={...defaultSettings(),laborHourly:20,machines:[{id:1,code:"ST1",washCost:10,washMinutes:30,gainPerShift:100},{id:2,code:"FILL1",washCost:5,washMinutes:15,gainPerShift:""}]};
...
test("zero confirmed washes is a valid actual",()=>{const r=calc(sampleEvidence,config,{washes:[{productionId:1,count:0,minutes:0}]});assert.equal(r.actualWash,0);assert.equal(r.actualTotal,410);});
...
test("filling uses bulk cost once, plus packaging/labor/wash, divided by good pieces",()=>{
 ...
 assert.equal(resolved.commercial.bulkTransferCost,210);assert.equal(resolved.productActualTotal,340);assert.equal(resolved.unitCost,3.4);
 assert.equal(resolved.directActualTotal,130);assert.equal(bulk.directActualTotal+resolved.directActualTotal,550);
});
