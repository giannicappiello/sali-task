...
export default function MenuManagement() {
  ...
  const load = useCallback(async () => {
    const results = await Promise.all([
      ...
      supabase.from("workspace_moduli").select("codice,nome,descrizione,icona,area,aree,attivo,mostra_menu,ordine").order("ordine").order("nome"),
  ...
  const selectedAreaModules = associations.areaModules.get(selectedArea) || [];
  const selectedAreaScreens = associations.areaScreens.get(selectedArea) || [];
  ...
  const visibleMenus = useMemo(() => catalog.menus.filter((menu) => {
    ...
    const searchableModules = moduleLinks.map((link) => `${link.module?.nome || ""} ${link.module?.codice || ""} ${moduleAreaCodes(link.module).join(" ")}`).join(" ");
  ...
  const menuPickerModules = useMemo(() => {
    ...
    return filterKeepingSelected(available, pickerSearch, ["nome", "codice", "descrizione", "aree", (item) => moduleAreaCodes(item).map((code) => associations.areaByCode.get(code)?.nome || code).join(" ")], menuForm.moduli);
  }, [associations.areaByCode, catalog.modules, menuForm.moduli, pickerSearch]);
