...
test("migration V3 è additiva, fail-closed e preserva v1/v2", async () => {
  ...
  for (const table of ["workspace_v3_previews", "workspace_v3_confirmation_sagas", "workspace_v3_material_commitments", "workspace_v3_purchase_requirements", "workspace_v3_purchase_documents", "workspace_v3_outbox", "workspace_v3_audit"]) assert.match(sql, new RegExp(`create table if not exists public\\.${table}`));
