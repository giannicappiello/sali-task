...
import WorkspaceDataBlock from "./WorkspaceDataBlock";
...
export default function WorkspaceScreenComposition({ layout, children, preview = false, requireSystemContent = Boolean(children) }) {
  ...
  return <div className={`workspace-composition ${preview ? "is-preview" : ""}`}>
    {normalized.blocks.map((block) => {
      ...
      if (block.type === "notice") return <section className={`${className} workspace-composition-notice ${block.variant || "info"}`} key={block.id}><h2>{block.title || "Informazione"}</h2>{block.text ? <p>{block.text}</p> : null}</section>;
      if (block.type === "kpi" || block.type === "data-table") return <div className={className} key={block.id}><WorkspaceDataBlock block={block} preview={preview}/></div>;
