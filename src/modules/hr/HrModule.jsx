...
export default function HrModule({ configuration = false }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('workspace:hr-changed', refresh);
  ...
  function contract(member) {
    ...
    show({ title: `Accordi · ${member.name}`, description: 'Ogni salvataggio crea una versione con decorrenza. Questi dati sono accessibili soltanto agli admin.',
      values: { weekly_hours: 40, start_time: '08:00', end_time: '17:00', break_minutes: 60, weekdays: [1, 2, 3, 4, 5], agreed_pay: '', pay_period: 'month', overtime_mode: 'paid', overtime_rate: 0, overtime_percent: 0, site_id: data.sites[0]?.id || '', ...current, effective_from: today },
      fields: [field('effective_from', 'Decorrenza', 'date', { min: today }), field('site_id', 'Sede', 'select', { options: data.sites.map((s) => [s.id, s.name]) }), field('weekly_hours', 'Ore pattuite / settimana', 'number', { min: 0.01, max: 80 }), field('weekdays', 'Giorni lavorativi', 'weekdays'), field('start_time', 'Orario pattuito · ingresso', 'time'), field('end_time', 'Orario pattuito · uscita', 'time'), field('break_minutes', 'Pausa non lavorata · minuti', 'number', { min: 0, max: 480 }), field('agreed_pay', 'Compenso pattuito · EUR', 'number', { min: 0 }), field('pay_period', 'Periodicità compenso', 'select', { options: [['month', 'Mensile'], ['hour', 'Oraria'], ['year', 'Annuale']] }), field('overtime_mode', 'Gestione straordinario', 'select', { options: [['paid', 'Retribuzione con approvazione'], ['bank', 'Banca ore con approvazione'], ['disabled', 'Non abilitato']] }), field('overtime_rate', 'Tariffa base straordinario · EUR/ora', 'number', { min: 0 }), field('overtime_percent', 'Maggiorazione straordinario · %', 'number', { min: 0, max: 500 })] },
    (values) => { if (!values.weekdays.length) throw new Error('Seleziona almeno un giorno.'); return configure('contract', { ...values, user_id: member.user_id }); });
  }
  function site(existing) {
    show({ title: existing ? 'Modifica sede' : 'Nuova sede', values: { checkin_radius: 30, checkout_radius: 100, ...existing, auto_checkout: existing?.auto_checkout === false ? 'false' : 'true' }, fields: [field('name', 'Nome sede'), field('latitude', 'Latitudine', 'number', { min: -90, max: 90 }), field('longitude', 'Longitudine', 'number', { min: -180, max: 180 }), field('checkin_radius', 'Raggio check-in · metri', 'number', { min: 10, max: 100 }), field('checkout_radius', 'Raggio checkout · metri', 'number', { min: 50, max: 1000 }), field('auto_checkout', 'Checkout automatico di supporto', 'select', { options: [['true', 'Attivo'], ['false', 'Disattivo']] })] }, (values) => configure('site', { ...values, id: existing?.id, auto_checkout: values.auto_checkout === 'true' }));
  }
