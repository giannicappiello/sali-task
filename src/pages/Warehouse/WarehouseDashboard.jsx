...
export default function WarehouseDashboard() {
  ...
  return <ModuleContainerLayout icon={BarChart3} eyebrow="Modulo Workspace" title="Magazzino" description="Giacenze inventariali storiche per articolo e singolo magazzino." backFallback="/home">
    <div className="warehouse-dashboard-page">
      ...
      {!loading && !error && data.snapshotAvailable ? <>
        ...
        <section className="warehouse-dashboard-kpis">
          <article><span>Articoli<InfoTip text="Numero di codici articolo distinti nel dataset filtrato per data, magazzino, tipologia, UDM, ricerca e stato giacenza." label="Articoli" /></span><strong>{quantityFormat.format(summary.articles || 0)}</strong><small>{quantityFormat.format(summary.locations || 0)} righe articolo-magazzino</small></article>
        ...
        <section className="warehouse-dashboard-charts"><Donut title="Valore per tipologia" info="Ripartizione del valore positivo per tipologia articolo, calcolata server-side sullo stesso dataset filtrato della tabella." items={breakdown.byType || []} valueKey="stock_value" labelKey="article_type" formatter={currencyFormat.format} /><Donut title="Articoli per tipologia" info="Codici articolo distinti per tipologia nel dataset filtrato." items={breakdown.byType || []} valueKey="articles" labelKey="article_type" formatter={quantityFormat.format} /><Donut title="Articoli per magazzino" info="Codici articolo distinti per singolo magazzino; con Tutti ogni magazzino resta separato." items={(breakdown.byWarehouse || []).map((item) => ({ ...item, label: `MAG-${item.warehouse_number}` }))} valueKey="articles" labelKey="label" formatter={quantityFormat.format} /></section>
        ...
        <p className="warehouse-dashboard-note">Ogni riga rappresenta Articolo + Magazzino. Le giacenze negative restano consultabili e valgono zero esclusivamente nelle valorizzazioni economiche.</p>
        <section className="warehouse-table-card" aria-label="Elenco giacenze per articolo e magazzino"><header><div><strong>Elenco giacenze</strong><span>{quantityFormat.format(data.totalRows || 0)} risultati</span></div><small>Data inventariale: {asOfDate} · Ultimo aggiornamento separato dalla competenza.</small></header><div className="warehouse-table-scroll"><table><thead><tr><th>Articolo</th><th>Magazzino</th><th>UDM</th><th>Giacenza</th><th>Impegnato</th><th>Disponibile</th><th>Costo ultimo</th><th>Valore</th><th>Stato</th><th>Ultimo aggiornamento</th></tr></thead><tbody>{data.rows.map((row) => { const status = stockStatus(row); return <tr key={`${row.article_code}:${row.warehouse_number}`}><td><strong>{row.article_code}</strong><small>{row.description || "Descrizione non disponibile"}</small></td><td><strong>MAG-{row.warehouse_number}</strong>{row.warehouse_name ? <small>{row.warehouse_name}</small> : null}</td><td>{row.unit_of_measure}</td><td>{quantityFormat.format(row.on_hand)}</td><td>{row.committed === null ? "—" : quantityFormat.format(row.committed)}</td><td>{row.available === null ? "—" : quantityFormat.format(row.available)}</td><td>{Number(row.unit_cost) > 0 ? costFormat.format(row.unit_cost) : <span className="warehouse-missing-cost">Da valorizzare</span>}</td><td><strong>{currencyFormat.format(row.stock_value)}</strong></td><td><span className={`warehouse-status ${status.tone}`}>{status.label}</span></td><td>{formatDate(row.captured_at)}</td></tr>; })}</tbody></table></div><footer className="warehouse-pagination"><button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Precedente</button><span>Pagina {page} di {pages}</span><button type="button" disabled={page >= pages} onClick={() => setPage((value) => value + 1)}>Successiva</button></footer></section>
      </> : null}
    </div>
