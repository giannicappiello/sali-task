...
export default function MexalAgents() {
  ...
  return <div className="mexal-page">
    ...
    <section className="mexal-hero"><div className="mexal-hero-main"><div className="mexal-logo"><UserRoundCog size={30} /></div><div><div className="mexal-title-line"><h1>Agenti Mexal</h1></div><p>Sincronizzazione Mexal → Workspace, responsabili associati e attivazione degli accessi.</p></div></div><button className="orders-primary" type="button" disabled={busy === "sync"} onClick={syncAgents}>{busy === "sync" ? <RefreshCw className="spin" size={18} /> : <RefreshCw size={18} />} Sincronizza agenti</button></section>
    ...
    <section className="mexal-table-panel"><div className="mexal-section-heading"><div><h3>Elenco agenti</h3><p>Nome, cognome, codice, email e telefono vengono aggiornati da Mexal. Responsabile e accesso restano sotto controllo Workspace.</p></div><label className="orders-search"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cerca agente..." /></label></div>
      {loading ? <p>Caricamento agenti...</p> : <div className="mexal-table-scroll"><table className="mexal-history-table"><thead><tr><th>Codice</th><th>Agente</th><th>Email</th><th>Telefono</th><th>Responsabile associato</th><th>Accesso Workspace</th><th>Ultimo sync</th><th>Azioni</th></tr></thead><tbody>{filtered.length === 0 ? <tr><td colSpan="8">Nessun agente trovato.</td></tr> : filtered.map((agent) => <tr key={agent.id}><td><strong>{agent.codice}</strong></td><td>{`${agent.nome || ""} ${agent.cognome || ""}`.trim() || "—"}</td><td>{agent.email || "—"}</td><td>{agent.telefono || "—"}</td><td><select disabled={busy === agent.id} value={agent.responsabile_utente_id || ""} onChange={(event) => setResponsible(agent.id, event.target.value)}><option value="">Nessun responsabile</option>{users.filter((user) => user.id !== agent.workspace_utente_id).map((user) => <option key={user.id} value={user.id}>{`${user.nome || ""} ${user.cognome || ""}`.trim() || user.email}</option>)}</select></td><td>{agent.accesso_workspace_attivo ? <span className="orders-status inviato-mexal">ATTIVO</span> : <span className="orders-status bozza">NON ATTIVO</span>}</td><td>{agent.ultimo_sync_mexal ? new Date(agent.ultimo_sync_mexal).toLocaleString("it-IT") : "—"}</td><td><div className="mexal-manual-actions">{agent.accesso_workspace_attivo ? <><button className="orders-secondary" type="button" disabled={busy === agent.id} onClick={() => activate(agent)}><KeyRound size={16} /> Cambia password</button><button className="orders-danger" type="button" disabled={busy === agent.id} onClick={() => disable(agent)}>Disattiva</button></> : <button className="orders-primary" type="button" disabled={busy === agent.id || !agent.email} onClick={() => activate(agent)}><ShieldCheck size={16} /> Attiva accesso</button>}</div></td></tr>)}</tbody></table></div>}
    </section>
