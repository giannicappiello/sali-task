...
begin
  ...
  select count(*)
    into reparti_richiesti
  from (
    select distinct reparto_id
    from unnest(coalesce(p_reparto_ids, array[]::uuid[])) reparto_id
  ...
  select count(*)
    into reparti_trovati
  from public.reparti r
  where r.id in (
    select distinct reparto_id
    from unnest(p_reparto_ids) reparto_id
    where reparto_id is not null
  )
  ...
  returning id into conversazione_id;

  insert into public.chat_partecipanti (conversazione_id, utente_id)
  select conversazione_id, membri.utente_id
  from (
