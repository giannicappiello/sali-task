...
export function formatMexalDocumentDate(value, format = "dd/mm/yyyy") {
  const input = text(value);
  const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(input);
  if (!match) throw new Error("data_documento obbligatoria e valida nel formato YYYY-MM-DD.");
  const [, year, month, day] = match;
  const date = new Date(Date.UTC(Number(year), Number(month) - 1, Number(day)));
  if (date.getUTCFullYear() !== Number(year) || date.getUTCMonth() !== Number(month) - 1 || date.getUTCDate() !== Number(day)) {
    throw new Error("data_documento non valida.");
  }
  const italian = `${day}/${month}/${year}`;
  if (format === "dd/mm/yyyy") return italian;
  if (format === "yyyymmdd") return `${year}${month}${day}`;
  if (format === "iso") return input;
  if (format === "typed-array-dd/mm/yyyy") return [[1, italian]];
...
export function buildMexalOrderDocument(order, kind, lines, { serie = 1, magazzino = 5, notaFormat = "typed-array", dateFormat = "dd/mm/yyyy" } = {}) {
  ...
  return compact({
    sigla: "OC", serie: number(serie), numero: 0, cod_conto: text(order.codice_cliente), data_documento: formatMexalDocumentDate(order.data_ordine, dateFormat),
