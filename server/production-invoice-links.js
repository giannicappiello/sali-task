...
export function automaticInvoiceMatches(e, allEvidence, headers, lines, allocations) {
 ...
 for(const h of headers){
  ...
  if(unique.length!==1||!unique[0].reference.startsWith("OC+")||!unique[0].year)continue;
  const ref=unique[0];
  ...
  for(const l of lines.filter(l=>l.fattura_id===h.id&&l.codice_articolo===e.articleCode)){
   ...
   result.push({id:`auto-${l.id}`,document:{id:h.id,sigla:h.sigla,serie:h.serie,numero:h.numero,data_documento:h.data_documento},amount:Number(l.valore_netto),quantity:null,source:"OCT originale e articolo univoci"});
  }
 }
 return result;
}
