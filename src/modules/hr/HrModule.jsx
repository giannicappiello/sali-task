...
import { agreementValues, AGREEMENT_FIELDS } from './hrAgreements';
  ...
  function editMember(member) {
    ...
    show({ title: `Scheda e accordi · ${member.name}`, description: 'Scheda dipendente e accordi riservati agli admin. La decorrenza può essere pregressa; i campi descrittivi degli accordi sono facoltativi e accettano testo libero. Con Gestisci Straordinario Separatamente le ore vengono escluse dai totali ordinari dell’Excel e riportate a parte. Le modifiche agli accordi conservano le versioni precedenti.',
      ...
      async (values, key) => {
        ...
        await hrRpc('workspace_hr_save_employee', { p_key: key, p_data: { user_id: member.user_id, employee_code: values.employee_code, reviewer_ids: values.reviewer_ids || [], manager: values.manager === 'true', contract_id: current?.id || null, ...(changed ? { contract: { ...Object.fromEntries(Object.keys(initial).map(k => [k, values[k]])), overtime_mode: current?.agreement_fields?.overtime_mode ?? current?.overtime_mode ?? '' } } : {}) } });
        await load();
      });
  }
