const LOCAL_PATHS = new Set(["/produzione/rdp-workbench", "/produzione/fabbisogni-acquisto", "/produzione/diagnostica", "/revisione-priorita-produzione"]);
...
export function configuredProductionSections(remote, screens, links, { hasPermission, isAdminUser, customerScoped, hasScreenAccess }) {
  ...
  return links.filter(l => l.visibile_menu && l.modulo_codice === "progremes")
    ...
    .flatMap(link => {
      ...
      if ((path.endsWith("/fabbisogni-acquisto") || path === "/revisione-priorita-produzione") && customerScoped) return [];
      return [{ code: path.split("/").at(-1), path, name: screen.nome, description: screen.descrizione, workspaceLocal: true }];
    });
}
