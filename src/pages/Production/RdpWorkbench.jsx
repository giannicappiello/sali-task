...
function PreviewDialog({ preview, busy, onCancel, onConfirm }) {
  ...
  return <div className="rdp-dialog-backdrop" role="presentation"><section className="rdp-dialog" role="dialog" aria-modal="true" aria-label="Anteprima RdP">
    ...
    <div className="rdp-dialog-actions"><button type="button" className="secondary-action" onClick={onCancel} disabled={busy}>Annulla</button><button type="button" className="primary-action" onClick={onConfirm} disabled={busy}>{busy ? <><RefreshCw className="rdp-spin" size={17}/>Invio in corso…</> : <><Send size={17}/>Crea RdP</>}</button></div>
  </section></div>;
}
...
export default function RdpWorkbench({ onBack }) {
  ...
  async function load() { setLoading(true); setError(""); try { const payload = await callWorkbench(accessToken, "progremes_workbench_list"); setData(payload.items || []); } catch (e) { setError(e.message); } finally { setLoading(false); } }
  useEffect(() => {
    if (!accessToken) return undefined;
    let active = true;
    callWorkbench(accessToken, "progremes_workbench_list")
      .then((payload) => { if (active) setData(payload.items || []); })
  ...
  async function createPreview() { if (!selected.length || busy) return; setBusy(true); setError(""); try { setPreview(await callWorkbench(accessToken, "progremes_production_preview", { orderIds: selected })); } catch (e) { setError(e.message); } finally { setBusy(false); } }
  async function sendRequest() { if (!preview || busy) return; setBusy(true); setError(""); try { const response = await callWorkbench(accessToken, "progremes_production_request", { orderIds: selected, snapshotId: preview.snapshot.id }); setResult(response); setPreview(null); setSelected([]); await load(); } catch (e) { setError(e.code === "DEMAND_CHANGED" ? "L’OCT è cambiato dopo l’anteprima: ripetere il precheck." : e.message); } finally { setBusy(false); } }
  ...
  return <div className="production-page rdp-workbench">
    ...
    <PreviewDialog preview={preview} busy={busy} onCancel={() => setPreview(null)} onConfirm={sendRequest}/>
