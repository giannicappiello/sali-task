...
function PlanPreview({ decision, busy, onApprove, onRegenerate, onCancel }) {
  ...
  return <section className="crm-plan">
    ...
    <div className="crm-plan-actions">
      <button className="crm-primary" type="button" disabled={busy || plan.readyForApproval === false} onClick={onApprove}><CheckCircle2 size={17} />Approva e crea progetto</button>
      <button className="crm-secondary" type="button" disabled={busy} onClick={onRegenerate}><RefreshCw size={17} />Rigenera</button>
      <button className="crm-secondary" type="button" disabled={busy} onClick={onCancel}><XCircle size={17} />Annulla</button>
    </div>
