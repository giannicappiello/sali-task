...
export function quantitiesForOrderLine(line, availability, confirm) {
  if (!confirm) {
    ...
    return { quantita_disponibile: confirmed, quantita_ocm: confirmed, quantita_ocx: Math.max(0, Number(line.quantita) - confirmed) };
  }
  ...
  return { quantita_disponibile: result.confirmedQuantity, quantita_ocm: result.confirmedQuantity, quantita_ocx: result.missingQuantity };
}
