...
export function validateSettings(settings) {
 ...
 for(const p of settings.prices||[]) if(![p.articleCode,p.customerCode,p.orderNumber,p.price].every(v=>v==null||String(v).trim()==="") && (!p.articleCode?.trim() || !p.unit?.trim() || number(p.price)===null || p.price<0)) throw new Error("Prezzo stimato: articolo, unità e valore obbligatori.");
...
export function scheduledHours(start,end,settings) {
 ...
 let total=0; const day=new Date(from);day.setUTCHours(0,0,0,0);if(!settings.companyCalendar)day.setUTCDate(day.getUTCDate()-1);
 for(let count=0;day<to&&count<36600;count++,day.setUTCDate(day.getUTCDate()+1)){
  for(const s of costDayShifts(settings,day.toISOString().slice(0,10))){
