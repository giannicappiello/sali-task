  ...
  return <>
    ...
    {open && <div className="orders-preparation-backdrop" role="dialog" aria-modal="true" aria-label="Preparazione nuovo ordine">
      <div className="orders-preparation-modal"><h2>Preparazione nuovo ordine</h2>
        ...
        {!running && result && <div className="orders-preparation-actions"><button type="button" className="orders-primary" onClick={prepare}>Riprova</button><button type="button" onClick={() => setOpen(false)}>Annulla</button>{canContinue && <button type="button" onClick={continueAnyway}>Continua comunque</button>}{!result.interrupted && <button type="button" onClick={continueAnyway}>Apri nuovo ordine</button>}</div>}
      </div></div>}
  </>;
}
