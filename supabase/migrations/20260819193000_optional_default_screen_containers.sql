...
-- Un modulo senza schermata iniziale è un contenitore navigabile a card.
update public.workspace_moduli modulo
set tipo='contenitore',
    percorso='/moduli/' || modulo.codice,
    aggiornato_il=now()
where modulo.tipo<>'contenitore'
  and not exists (
    select 1
    from public.workspace_moduli_schermate collegamento
    where collegamento.modulo_codice=modulo.codice
      and collegamento.predefinita=true
  );
