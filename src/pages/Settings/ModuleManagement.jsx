...
export default function ModuleManagement() {
  ...
  const load = useCallback(async () => {
    ...
    } else if (targetScreen) setScreenForm({ ...targetScreen, moduli: (linksResult.data || []).filter(link => link.schermata_codice === targetScreen.codice).map(link => link.modulo_codice) });
  ...
  const visibleScreens = useMemo(() => screens.filter((item) => {
    ...
    const areaNames = screenAreaCodes(item).map((code) => associations.areaByCode.get(code)?.nome || code).join(" ");
    return matchesProvider && matchesStatus && screenMatchesArea(item, areaFilter)
      && matchesAssociationStatus(associationCount, associationStatus)
      && matchesWorkspaceSearch(item, search, ["nome", "codice", "descrizione", "aree", "area", "provider", "percorso", screenDestination, () => areaNames]);
  ...
  function editScreen(screen) {
    setScreenForm({ ...screen, moduli: links.filter(link => link.schermata_codice === screen.codice).map(link => link.modulo_codice) });
    setSelectedCode("");
  }
  ...
  async function saveScreen(event) {
    ...
    setBusy(true);
    const { error } = await supabase.rpc("admin_save_workspace_screen_modules", { target_module_codes: screenForm.moduli || [], target_screen: {
      codice: screenForm.codice,
      nome: screenForm.nome.trim(),
      descrizione: screenForm.descrizione?.trim() || null,
      aree: screenAreaCodes(screenForm),
  ...
  return (
    <div className="module-settings-page">
      ...
      {view === "modules" ? (
        ...
        <div className="module-composer-grid">
          <section className="module-catalog-list" tabIndex={0} aria-label="Elenco elementi da configurare">
            {visibleScreens.map((screen) => { const moduleAssociations=associations.screenLinks.get(screen.codice)||[]; const ScreenIcon=getModuleIcon(screen.icona,screen.protetta?ShieldCheck:Monitor); return <div className={`module-catalog-item catalog-with-associations ${screenForm?.codice === screen.codice ? "active" : ""}`} key={screen.codice}><button type="button" className="module-catalog-select" onClick={() => editScreen(screen)}><span className="module-list-icon"><ScreenIcon /></span><span><strong>{screen.nome}</strong><small>{screen.codice} · {screenRouteSummary(screen)}</small><span className="catalog-relations">Aree: {screenAreaCodes(screen).map((code) => associations.areaByCode.get(code)?.nome || code).join(", ")}</span><span className="catalog-relations">Moduli: {moduleAssociations.slice(0,2).map((link)=>link.module?.nome).filter(Boolean).join(", ")||"nessuno"}{moduleAssociations.length>2?` · + ${moduleAssociations.length-2}`:""}</span></span><AssociationBadge associated={moduleAssociations.length>0} orphanLabel="Non raggiungibile"/><Pencil size={16} /></button><Link className="module-preview-link" to={`/settings/layout-builder/screen/${encodeURIComponent(screen.codice)}`} title={`Modifica la struttura della schermata ${screen.nome}`}><ExternalLink size={16} /><span>Apri</span></Link></div>; })}
