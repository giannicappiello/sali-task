...
const TABS = [
  ...
  ["history", "Storico RdP"], ["planning", "Planning"],
];
...
function V3Panel({ v3, canDecide, busy, onPreview, onConfirm }) {
  ...
  return <section className="rdp-decisions rdp-v3-panel">
    <button type="button" className="primary-action rdp-v3-recalculate" onClick={onPreview} disabled={busy || !v3.flags?.["workspacemes.v4.preview"]}>{busy ? "RICALCOLO RDP…" : "RICALCOLA RDP"}</button>
    {preview && <>
...
function DetailPanel({ detail, onClose, onDiagnostics, canDecide, canCancel, onDecision, onCancel, onV3Preview, onV3Confirm, busy }) {
  ...
  return <div className="rdp-detail-backdrop" role="presentation" onMouseDown={onClose}><section className="rdp-detail" role="dialog" aria-modal="true" aria-label="Dettaglio RdP e OCT" onMouseDown={(event) => event.stopPropagation()}>
    ...
    {detail.request && <div className="rdp-request-meta"><span>Stato {badge(detail.request.workspace_status || detail.request.stato, detail.request.stage === "blocked" ? "red" : "blue")}</span><span>Creata {formatDate(detail.request.created_at, true)}</span><span>Tentativi {detail.request.attempt_count ?? 0}</span><span>Contratto V4</span>{detail.cancellation?.allowed && <button type="button" className="rdp-cancel-action" onClick={onCancel} disabled={busy || !canCancel}><Ban size={16}/>Annulla RdP</button>}{detail.cancellation?.allowed && !canCancel && <small>Permesso rdp.cancel richiesto.</small>}</div>}
...
function DecisionDialog({ decision, busy, onClose, onConfirm }) {
  ...
  const v3 = ["v3", "v4"].includes(decision.kind);
  return <div className="rdp-dialog-backdrop"><section className="rdp-dialog" role="dialog" aria-modal="true" aria-label="Conferma pianificazione"><header><div><span className="rdp-eyebrow">Decisione operatore</span><h2>Genera ordini di produzione</h2></div><button type="button" onClick={onClose} disabled={busy}><X/></button></header><div className="rdp-no-netting"><ShieldAlert/><div><strong>Impatto produttivo</strong><p>{v3 ? "ProgreMES creerà OdP e impegni FIFO. Se la preview certifica carenze, Workspace genererà automaticamente i relativi fabbisogni di acquisto." : `ProgreMES creerà o confermerà la pianificazione per ${decision.articleCode}, quantità ${decision.quantity} ${decision.productionUom || decision.octUom}.`}</p></div></div>{v3 && <label className="rdp-cancel-reason"><span>Motivazione obbligatoria</span><textarea rows={3} minLength={5} maxLength={1000} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Indica il motivo della decisione produttiva…"/></label>}<div className="rdp-dialog-actions"><button type="button" className="secondary-action" onClick={onClose} disabled={busy}>Annulla</button><button type="button" className="primary-action" onClick={() => onConfirm(reason.trim())} disabled={busy || (v3 && reason.trim().length < 5)}>{busy ? "Conferma…" : "Conferma e genera OP"}</button></div></section></div>;
}
...
export default function RdpWorkbench() {
  ...
  return <div className="production-page rdp-workbench">
    <header className="rdp-header"><div><span className="rdp-eyebrow">WorkspaceMES</span><h1>RdP Workbench</h1><p>Gestione OCT, richieste di produzione, analisi MES e decisioni operative.</p></div><div className="rdp-header-controls"><nav className="rdp-tabs" aria-label="Stati Workbench">{TABS.map(([code,label]) => <button type="button" key={code} className={tab === code ? "active" : ""} onClick={() => { if (code === "purchasing") navigate("/produzione/fabbisogni-acquisto"); else if (code === "planning") navigate("/produzione/progremes.Planning"); else setTab(code); }}>{label}{!["purchasing", "planning"].includes(code) && <span>{data.filter((row) => row.stage === code).length}</span>}</button>)}</nav><div className="rdp-header-actions"><BackgroundSyncStatus refresh={octRefresh}/></div></div></header>
    {tab !== "purchasing" && <section className={`rdp-toolbar ${tab === "evaluation" ? "rdp-toolbar-evaluation" : ""}`}><label className="rdp-quick-search"><Search size={17}/><input value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} placeholder="Cerca OCT, cliente, stato…"/></label><select value={filters.ready} onChange={(e) => setFilters({ ...filters, ready: e.target.value })}><option value="">Pronti e bloccati</option><option value="ready">Solo pronti</option><option value="blocked">Solo bloccati</option></select><button type="button" className="secondary-action rdp-toolbar-refresh" onClick={refreshOctOrders} disabled={loading || octRefreshRunning}><RefreshCw className={loading || octRefreshRunning ? "rdp-spin" : ""} size={17}/>Aggiorna</button>{tab === "evaluation" && <div className="rdp-selection-bar"><span><strong>{selected.length}</strong> OCT selezionati</span><button type="button" className="primary-action rdp-preview-action" onClick={createPreview} disabled={!canCreate || !sendEnabled || !selected.length || selectionBlocked || busy}>{busy ? "Verifica…" : "Verifica e crea anteprima"}</button>{!canCreate && <small>Permesso rdp.create richiesto.</small>}{!sendEnabled && <small>Invio RdP Production non disponibile: verificare i gate nel Centro Diagnostico.</small>}{selectionBlocked && <small>Rimuovere gli OCT bloccati prima di creare la RdP.</small>}</div>}</section>}
