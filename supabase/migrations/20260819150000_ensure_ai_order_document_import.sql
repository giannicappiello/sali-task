...
create index if not exists ai_ordini_acquisizioni_utente_data_idx on public.ai_ordini_acquisizioni (utente_id,creata_il desc);
...
create policy "users read own AI order acquisitions" on public.ai_ordini_acquisizioni for select to authenticated
...
returns jsonb language sql stable security definer set search_path=public as $$
  with me as (
    ...
    select max(case coalesce(p.livello,'nessuno') when 'conferma' then 3 when 'bozza' then 2 when 'analisi' then 1 else 0 end) as level_rank,
      coalesce(bool_or(p.riconoscimento_immagini),false) as vision
    from departments d left join public.ai_reparti_moduli p on p.reparto_id=d.reparto_id and p.modulo_codice=target_module_code
  ), general as (
    ...
    select me.*,coalesce(o.livello,'eredita') as override_level,o.riconoscimento_immagini as override_vision,
      coalesce(rm.livello_accesso,'lettura') as business_level,policy.level_rank,policy.vision as module_vision,general.vision as general_vision
    ...
    select *,case when is_admin then 'conferma'
      ...
      when level_rank=3 then 'conferma' when level_rank=2 then 'bozza' when level_rank=1 then 'analisi' else 'nessuno' end as ai_level
    from effective
  )
  select coalesce((select jsonb_build_object(
    'allowed',is_admin or (public.workspace_module_enabled_for_user(id,'assistente_ai')
      and public.workspace_module_enabled_for_user(id,target_module_code)
      and business_level in ('scrittura','amministrazione') and ai_level in ('bozza','conferma')
      and general_vision and coalesce(override_vision,module_vision,false)),
    'level',ai_level,
