...
export default function PrivateProductionCostReports(){
 ...
 const searchCard=<section className="pc-panel pc-private-search"><div className="pc-fields"><label className="pc-field"><span>Ricerca RdP, OCT, cliente o prodotto</span><input value={query} onChange={e=>setQuery(e.target.value)}/></label><button disabled={loading} onClick={()=>{setLoading(true);setError('');setSelected(null);setRevision(v=>v+1);}}>Aggiorna</button></div></section>;
