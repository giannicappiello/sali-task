...
export function buildAssistantChartSvg(content, { logoHref = "/progre-logo-white.png" } = {}) {
  ...
  const bars = chart.points.map((point, index) => {
    ...
    return `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${barWidth.toFixed(1)}" height="${barHeight.toFixed(1)}" rx="5" fill="#126bff"/><text x="${(x + barWidth / 2).toFixed(1)}" y="${Math.max(margin.top - 8, y - 8).toFixed(1)}" text-anchor="middle" font-family="Arial" font-size="13" fill="#344054">${escapeXml(new Intl.NumberFormat("it-IT", { useGrouping: 'always',  maximumFractionDigits: 2 }).format(point.value))}</text><text transform="translate(${(x + barWidth / 2).toFixed(1)} ${margin.top + plotHeight + 18}) rotate(40)" text-anchor="start" font-family="Arial" font-size="13" fill="#475467">${escapeXml(compactLabel(point.label))}</text>`;
  }).join("");
