...
const EMPTY_HEADING = { id: "", name: "", code: "", description: "", companyBrand: "", kind: "carta_intestata", language: "it", format: "DOCX", validFrom: "", validTo: "", isDefault: false, notes: "" };
...
const EMPTY_FILTERS = { search: "", status: "all", brand: "all", format: "all", language: "all", usage: "all" };
const STATUS_LABELS = { draft: "Bozza", active: "Pubblicata / attiva", disabled: "Disattivata", archived: "Archiviata" };
...
export default function CompanyLetterheads() {
  ...
  const [dataset, setDataset] = useState("headings");
  ...
  const activeRules = useMemo(() => rules.filter((item) => item.active), [rules]);
  const unassociatedTypes = useMemo(() => documentTypes.filter((type) => !activeRules.some((rule) => rule.document_type_code === type.code)), [activeRules, documentTypes]);

  function showDataset(nextDataset, overrides = {}) {
    setDataset(nextDataset);
    setFilters({ ...EMPTY_FILTERS, ...overrides });
  }

  function editHeading(item) {
    setSelectedId("");
    setHeadingForm({ id: item.id, name: item.name, code: item.code, description: item.description || "", companyBrand: item.company_brand, kind: item.kind, language: item.language, format: item.format, validFrom: item.valid_from || "", validTo: item.valid_to || "", isDefault: item.is_default, notes: item.notes || "" });
  }
  ...
  async function saveHeading(event) {
    ...
    try {
      ...
      if (headingForm.id) params.p_letterhead_id = headingForm.id; else params.p_format = headingForm.format;
  ...
  return <>
    ...
    <section className="letterhead-kpis" aria-label="Riepilogo intestazioni"><button className={dataset==="headings"&&filters.status==="all"?"active":""} aria-pressed={dataset==="headings"&&filters.status==="all"} onClick={()=>showDataset("headings")}><strong>{headings.length}</strong><span>Intestazioni</span></button><button className={dataset==="headings"&&filters.status==="active"?"active":""} aria-pressed={dataset==="headings"&&filters.status==="active"} onClick={()=>showDataset("headings",{status:"active"})}><strong>{headings.filter((h)=>h.status==="active").length}</strong><span>Pubblicate / attive</span></button><button className={dataset==="unassociated"?"active":""} aria-pressed={dataset==="unassociated"} onClick={()=>showDataset("unassociated")}><strong>{unassociatedTypes.length}</strong><span>Tipi non associati</span></button><button className={dataset==="rules"?"active":""} aria-pressed={dataset==="rules"} onClick={()=>showDataset("rules")}><strong>{activeRules.length}</strong><span>Regole attive</span></button><button className={dataset==="signatures"?"active":""} aria-pressed={dataset==="signatures"} onClick={()=>showDataset("signatures")}><strong>{signatures.length}</strong><span>Firme in libreria</span></button></section>
