...
export default function ProductionCostConfiguration(){
 ...
 async function refreshHistory(){setBusy(true);setError("");try{setStationHistory((await action(token,"station-history",{settings})).history);}catch(e){setError(e.message);}finally{setBusy(false);}}
 async function refreshFillingHistory(){setBusy(true);setError("");try{setFillingHistory({...(await action(token,"filling-history",{settings})).history,checkedCalendar:calendarKey});}catch(e){setError(e.message);}finally{setBusy(false);}}
 ...
 {settings.laborRules?.station?.basis==="historical_productivity"&&<section className="pc-panel"><button type="button" onClick={refreshHistory}>Verifica media storica MES</button><StationHistorySummary history={stationHistory} hourly={settings.laborHourly}/></section>}
 {settings.laborRules?.filling?.basis==="historical_pieces"&&<section className="pc-panel"><button type="button" onClick={refreshFillingHistory}>Verifica produttività storica FILLING</button><FillingHistorySummary history={fillingHistory?.checkedCalendar===calendarKey?fillingHistory:null} hourly={settings.laborHourly}/></section>}
 ...
 <p className="pc-note">Anche per la media storica FILLING si usa il primo turno MES attuale esteso allo storico, con le chiusure MES sulle rispettive date; i turni Workspace dal secondo in poi sono aggiuntivi. La durata economica è sempre di 8 ore. Il costo a pezzi non aggiunge una seconda quota per turni o straordinari della singola lavorazione.</p>
 ...
 <p>In modalità media storica STATION il primo turno resta quello MES; i turni dal secondo in poi estendono il calendario economico e la media anche nello storico. Non sovrapporli al primo turno MES. Dopo le 17:00 nessun costo automatico fuori turno: lo straordinario va registrato nel dettaglio del consuntivo.</p><p>Con la regola iniziale (mezzo turno superiore), orario 09:00–16:00: fine alle 17:00 = 1 turno + 1 ora straordinaria; fine alle 11:00 del giorno successivo = 1,5 turni. I criteri FILLING sono indipendenti.</p>
 ...
 <button type="button" onClick={()=>update("shifts",[...settings.shifts,{name:"Secondo turno",start:"17:00",end:"23:00",breakMinutes:0,days:[1,2,3,4,5]}])}><Plus size={17}/>Aggiungi turno</button>
