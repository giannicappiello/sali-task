...
export default function RdpWorkbench({ commercial = false, compact = false, onNavigate, onReturnToPlan }) {
  ...
  async function openDetail(row) { setError(""); try { const loaded = await callWorkbench(accessToken, "progremes_workbench_detail", { orderId: row.orderId || row.id, requestId: row.requestId }); setDetail(loaded); if (canDecide && loaded.v3?.preview?.status === "READY" && loaded.v3.preview.snapshot?.confirmationRecovery && !loaded.v3.preview.snapshot.confirmationRecovery.rejected && !loaded.v3.preview.snapshot.confirmationRecovery.reconciliationRequired && !loaded.v3.saga) await confirmDecision({ kind: "v4", preview: loaded.v3.preview, request: loaded.request }); } catch (e) { setError(e.message); } }
