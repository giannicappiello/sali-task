...
- per articoli impegnati su altre lavorazioni usa la revisione coordinata MES_PRIORITY_MATERIALS, MES_PRIORITY_SIMULATE e MES_PRIORITY_REVISE. Vale per MP, packaging, componenti e bulk effettivamente prelevabile da magazzino, non solo codici MP. Un'eccedenza di prenotazioni è un avviso da risolvere scegliendo le origini: non dire che serve necessariamente una rettifica di magazzino. Anche missing=0 può richiedere minimumRelease>0. transfers.quantity è il disimpegno totale dall'origine; solo il necessario viene assegnato alla destinazione, il residuo torna libero. Puoi suggerire origini se richiesto, ma devi mostrare quantità, nuovi scoperti e date e attendere la conferma del riepilogo. Non usare il semplice MES_MATERIAL_REALLOCATE per aggirare il ricalcolo del planning. Non modificare giacenze, formule o consumi; proteggi lavorazioni avviate e materiali già consumati. Nessun avvio automatico né SL/CL. Non usare OP_UPDATE/RDP_UPDATE come bypass;
...
async function chat(auth, body) {
  ...
  const priorityTools = controlledTools.MES_PRIORITY_REVISE ? {
    ...
    MES_PRIORITY_MATERIALS: { description: "Legge tutti gli articoli prenotabili (MP, packaging, componenti e bulk da magazzino) e le origini. Mostra physical, totalReserved, overbooked, minimumRelease e vincoli. Overbooking è un avviso risolvibile tramite disimpegno, anche con missing=0. productionDependencies identifica dalla distinta confermata i semilavorati da produrre internamente: non inserirli nei trasferimenti, coprire le MP e mantenere miscelazione prima del confezionamento. Non dedurre il ruolo dal prefisso FP né dalla presenza nel Gantt. Non ridurre quantità di formula usando automaticamente scorte bulk. planningBlock e blockReason indicano OP e motivo esclusione: riportarli senza aggirare i controlli. Non applica trasferimenti.",
    ...
    MES_PRIORITY_SIMULATE: { description: "Simula disimpegno e revisione RdP/planning per tutti gli articoli prenotabili. transfers.quantity indica quanto togliere all’origine, non quanto assegnare alla destinazione. Selezionare almeno minimumRelease: comprende eccedenza prenotazioni e copertura destinazione. Consentito anche missing=0 e oltre lo scoperto; solo il necessario va alla destinazione, il residuo torna libero. Mai sottrarre da origini non eligible, consumate o avviate. Mostrare origini penalizzate, quantità, scoperti e nuove date. Nessuna applicazione: proporre poi MES_PRIORITY_REVISE con conferma esplicita del riepilogo.",
