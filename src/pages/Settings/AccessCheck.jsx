...
export default function AccessCheck() {
  ...
  return <section className="access-page">
    ...
    {selectedUser && <div className="access-audit-summary"><div><span className="access-avatar">{`${selectedUser.nome?.[0] || ""}${selectedUser.cognome?.[0] || ""}`}</span><span><strong>{fullName(selectedUser)}</strong><small>{selectedUser.ruoli?.nome || "Nessun ruolo"}</small></span></div><div><strong>{result.areas.filter((item) => item.allowed).length}</strong><span>Aree</span></div><div><strong>{result.modules.filter((item) => item.allowed).length}</strong><span>Moduli</span></div><div><strong>{result.screens.filter((item) => item.allowed).length}</strong><span>Schermate</span></div><div><strong>{result.exceptions?.length || 0}</strong><span>Eccezioni</span></div></div>}
