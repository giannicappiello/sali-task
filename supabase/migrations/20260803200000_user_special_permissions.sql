...
create table if not exists public.permessi_utente (
  utente_id uuid not null references public.utenti(id) on delete cascade,
  permesso_id uuid not null references public.permessi(id) on delete cascade,
  creato_il timestamptz not null default now(),
  primary key (utente_id, permesso_id)
);

alter table public.permessi_utente enable row level security;

drop policy if exists "users read own special permissions" on public.permessi_utente;
create policy "users read own special permissions"
on public.permessi_utente for select to authenticated
using (
  exists (
    select 1 from public.utenti u
    where u.id = permessi_utente.utente_id
...
drop policy if exists "admins manage user special permissions" on public.permessi_utente;
create policy "admins manage user special permissions"
on public.permessi_utente for all to authenticated
using (public.workspace_user_is_admin())
with check (public.workspace_user_is_admin());

-- Conserva l'operatività corrente copiando su ciascun utente tutti i permessi
-- finora ereditati dal suo ruolo. La tabella permessi_ruolo non viene eliminata.
insert into public.permessi_utente (utente_id, permesso_id)
select u.id, pr.permesso_id
from public.utenti u
join public.permessi_ruolo pr on pr.ruolo_id = u.ruolo_id
on conflict (utente_id, permesso_id) do nothing;
