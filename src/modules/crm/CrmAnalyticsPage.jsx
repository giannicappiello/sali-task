...
export default function CrmAnalyticsPage({ type }) {
  ...
  return <div className="crm-page">
    ...
    <div className="crm-kpi-grid"><Metric label="Conversione" value={metrics.conversion_rate == null ? "—" : `${metrics.conversion_rate}%`} text="Progetti vinti divisi per tutti i progetti chiusi; calcolo server-side sul periodo e sul perimetro autorizzato." to={period.withPeriod(`${config.basePath}/pipeline`, { view: "list" })} /><Metric label="Valore pipeline" value={formatMoney(metrics.pipeline_value)} text="Somma server-side del valore nominale dei progetti aperti." to={period.withPeriod(`${config.basePath}/pipeline`, { status: "open", view: "list" })} /><Metric label="Pipeline ponderata" value={formatMoney(metrics.weighted_value)} text="Valore moltiplicato per probabilità, per ogni progetto aperto." to={period.withPeriod(`${config.basePath}/pipeline`, { status: "open", view: "list" })} /><Metric label="Vinti" value={metrics.won || 0} text={`Progetti chiusi come vinti; valore finale ${formatMoney(metrics.won_value)}.`} to={period.withPeriod(`${config.basePath}/pipeline`, { outcome: "won", view: "list" })} /><Metric label="Persi" value={metrics.lost || 0} text={`Progetti chiusi come persi; valore finale ${formatMoney(metrics.lost_value)}.`} to={period.withPeriod(`${config.basePath}/pipeline`, { outcome: "lost", view: "list" })} /><Metric label="Durata ciclo" value={metrics.average_cycle_days == null ? "—" : `${metrics.average_cycle_days} gg`} text="Media dei giorni tra apertura e chiusura dei progetti nel periodo." to={period.withPeriod(`${config.basePath}/pipeline`, { outcome: "closed", view: "list" })} /></div>
