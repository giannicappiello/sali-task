...
export function PlanningLifecycleForm({ token, release = false, canUseAI = false, compact = false, initialSearch }) {
  ...
  return <div className="planning-lifecycle" data-screen-code={release ? "produzione.rilascio_odl" : "produzione.versioni_piano"} aria-busy={busy}>
    ...
    {!release && <><section className="plan-panel"><h2>Domande e ordini · {filtered.length}</h2><label>Ricerca totale<input type="search" placeholder="RdP, OP, stato, articolo…" value={query} onChange={e => setQuery(e.target.value)} /></label><p>Se non selezioni righe, la proposta considera l'intero orizzonte. Le modifiche manuali sono proposte, non spostamenti già applicati.</p>
      <div className="plan-table-wrap" tabIndex={0} role="region" aria-label="Domande e scelte manuali"><table className="plan-demands-table"><colgroup><col className="plan-col-selection" /><col className="plan-col-order" /><col className="plan-col-stage" /><col className="plan-col-date" /><col className="plan-col-date" /><col className="plan-col-date" /><col className="plan-col-start" /><col className="plan-col-resource" /></colgroup><thead><tr><th>Selezione</th><th>RdP / OP</th><th>Fase</th><th>Richiesta originale</th><th>Stima</th><th>Confermata</th>{!release && <><th>Non prima di</th><th>Impianto preferito</th></>}</tr></thead><tbody>{filtered.map(row => {
    ...
    {release && <nav className="plan-actions"><Link to="/produzione/progremes.PlanningProduction?workspaceMesWindow=1">Apri Pianificazione e produzione</Link></nav>}
