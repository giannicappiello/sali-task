...
import { packagingDepartmentAccess } from './packaging-access.js';
...
export async function canReadProductionSpecification(admin, profileId, pathname) {
  ...
  const results = await Promise.all(['progremes.OperatoreConfezionamento', 'progremes.OperatoreProduzione', 'progremes.Produzione'].map(screen =>
    admin.rpc('workspace_screen_level_for_user', { target_user_id: profileId, target_screen: screen })));
  for (const result of results) if (result.error) throw result.error;
  return results.some(result => allowedLevels.has(result.data)) || await packagingDepartmentAccess(admin, profileId) || await mixingDepartmentAccess(admin, profileId);
}
