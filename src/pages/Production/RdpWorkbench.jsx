...
function DetailPanel({ detail, onClose, onDiagnostics, canDecide, canCancel, onDecision, onCancel, onV3Preview, onV3Confirm, busy }) {
  ...
  return <div className="rdp-detail-backdrop" role="presentation" onMouseDown={onClose}><section className="rdp-detail" role="dialog" aria-modal="true" aria-label="Dettaglio RdP e OCT" onMouseDown={(event) => event.stopPropagation()}>
    ...
    {detail.request && <div className="rdp-request-meta"><span>Stato {badge(detail.request.workspace_status || detail.request.stato, detail.request.stage === "blocked" ? "red" : "blue")}</span><span>Creata {formatDate(detail.request.created_at, true)}</span><span>Tentativi {detail.request.attempt_count ?? 0}</span><span>Contratto V3</span>{detail.cancellation?.allowed && <button type="button" className="rdp-cancel-action" onClick={onCancel} disabled={busy || !canCancel}><Ban size={16}/>Annulla RdP</button>}{detail.cancellation?.allowed && !canCancel && <small>Permesso rdp.cancel richiesto.</small>}</div>}
...
export default function RdpWorkbench() {
  ...
  async function createV3Preview() { if (!detail?.request?.id || busy) return; setBusy(true); setError(""); setRdpFailure(null); try { const response = await callWorkbench(accessToken, "workspacemes_v3_preview", { requestId: detail.request.id }); const refreshedDetail = await callWorkbench(accessToken, "progremes_workbench_detail", { requestId: detail.request.id }); setDetail(refreshedDetail); const failure = v3RecalculationFailure(response) || v3RecalculationFailure({ status: refreshedDetail?.v3?.preview?.status, components: refreshedDetail?.v3?.components }); if (failure) setRdpFailure(failure); } catch (e) { setError(e.message); setRdpFailure({ code: e.code || "V3_PREVIEW_FAILED", message: e.message }); } finally { setBusy(false); } }
  ...
  return <div className="production-page rdp-workbench">
    ...
    <DetailPanel key={detail?.request?.id || detail?.orders?.map((order) => order.id).join(":") || "none"} detail={detail} onClose={() => setDetail(null)} onDiagnostics={openDiagnostic} canDecide={canDecide} canCancel={canCancel} onDecision={setDecision} onCancel={() => setCancelTarget(detail.request)} onV3Preview={createV3Preview} onV3Confirm={() => setDecision({ kind: "v3", preview: detail.v3.preview, request: detail.request })} busy={busy}/>
