...
const EMPTY_USER = { nome: "", cognome: "", email: "", telefono: "", password: "", ruolo_id: "", reparto_ids: [], responsabile_utente_id: "", mexal_agente_id: "", beauty_mexal_agente_id: "", attivo: true };
...
export default function AccessUsers() {
  ...
  const [data, setData] = useState({ users: [], roles: [], departments: [], userDepartments: [], agents: [], integrations: [], exceptions: [], aiOverrides: [], modules: [], areas: [], screens: [] });
  ...
  const loadData = useCallback(async () => {
    ...
    const [users, roles, departments, userDepartments, agents, integrations, exceptions, aiOverrides, modules, areas, screens] = results.map((result) => result.data || []);
    setData({ users, roles, departments, userDepartments, agents, integrations, exceptions, aiOverrides, modules, areas, screens });
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      mexal_agente_id: linkedAgent?.id || "", beauty_mexal_agente_id: beauty?.mexal_agente_id || "", attivo: selectedUser.attivo !== false,
    });
  ...
  }, [selectedUser, isCreating, data.userDepartments, data.agents, data.integrations, data.exceptions, data.aiOverrides]);
  ...
  async function saveUser() {
    ...
    const { data: response, error: userError } = await supabase.functions.invoke("admin-manage-user", { body: {
      ...
      password: form.password, ruolo_id: form.ruolo_id || null, reparto_id: primaryDepartment, responsabile_utente_id: form.responsabile_utente_id || null, attivo: form.attivo,
    } });
